...
A key frame animation's target values are defined by its KeyFrames property, which contains a collection of [Point3DKeyFrame](_Point3DKeyFrame.md) objects. Each [Point3DKeyFrame](_Point3DKeyFrame.md) defines a segment of the animation with its own target Value and [KeyTime](_KeyTime.md). When the animation runs, it progresses from one key value to the next at the specified key times.

There are three types of [Point3DKeyFrame](_Point3DKeyFrame.md) classes, one for each supported interpolation method: [LinearPoint3DKeyFrame](_LinearPoint3DKeyFrame.md), [DiscretePoint3DKeyFrame](_DiscretePoint3DKeyFrame.md), and [SplinePoint3DKeyFrame](_SplinePoint3DKeyFrame.md).

Unlike a [Point3DAnimation](_Point3DAnimation.md), a [Point3DAnimationUsingKeyFrames](_Point3DAnimationUsingKeyFrames.md) can have more than two of target values. You can also control the interpolation method of individual [Point3DKeyFrame](_Point3DKeyFrame.md) segments.
...
• [Point3DAnimationBase](_Point3DAnimationBase.md)
...
## From [Point3DAnimationBase](_Point3DAnimationBase.md)
