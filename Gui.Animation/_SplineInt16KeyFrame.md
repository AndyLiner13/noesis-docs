...
This class is used as part of a [Int16KeyFrameCollection](/Gui.Animation/_Int16KeyFrameCollection.md) in conjunction with a [Int16AnimationUsingKeyFrames](/Gui.Animation/_Int16AnimationUsingKeyFrames.md) to animate a Int16 property value along a set of key frames.

A key frame defines a segment of the [Int16AnimationUsingKeyFrames](/Gui.Animation/_Int16AnimationUsingKeyFrames.md) to which it belongs. Each key frame has a target Value and a [KeyTime](/Gui.Animation/_KeyTime.md). The [KeyTime](/Gui.Animation/_KeyTime.md) specifies the time at which the key frame's Value should be reached. A key frame animates from the target value of the previous key frame to its own target value. It starts when the previous key frame ends and ends when its own key time is reached.

Spline key frames like [SplineInt16KeyFrame](/Gui.Animation/_SplineInt16KeyFrame.md) creates a variable transition between values which is determined by the [KeySpline](/Gui.Animation/_KeySpline.md) property. Splined interpolation can be used to achieve more realistic "real world" timing effects such as acceleration and deceleration.
...
• [Int16KeyFrame](/Gui.Animation/_Int16KeyFrame.md)
...
## From [Int16KeyFrame](/Gui.Animation/_Int16KeyFrame.md)
