Source: https://www.noesisengine.com/docs/Gui.Animation._StringAnimationUsingKeyFrames.html

# StringAnimationUsingKeyFrames Class

## namespace [Noesis](../Gui.Core/_ClassHierarchy.md#noesis-namespace) | [MSDN](http://msdn.microsoft.com/en-us/library/system.windows.media.animation.stringanimationusingkeyframes.aspx)

Animates the value of a *String* property along a set of *KeyFrames*.

A key frame animation's target values are defined by its KeyFrames property, which contains a collection of [StringKeyFrame](_StringKeyFrame.md) objects. Each [StringKeyFrame](_StringKeyFrame.md) defines a segment of the animation with its own target Value and [KeyTime](_KeyTime.md). When the animation runs, it progresses from one key value to the next at the specified key times.
...
## From [StringAnimationBase](_StringAnimationBase.md)
