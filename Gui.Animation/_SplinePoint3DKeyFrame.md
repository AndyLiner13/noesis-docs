Source: https://www.noesisengine.com/docs/Gui.Animation._SplinePoint3DKeyFrame.html

# SplinePoint3DKeyFrame Class

## namespace [Noesis](../Gui.Core/_ClassHierarchy.md#noesis-namespace) | [MSDN](http://msdn.microsoft.com/en-us/library/system.windows.media.animation.splinepoint3dkeyframe.aspx)

Animates from the *Point3D* value of the previous key frame to its own *Value* using splined interpolation.

This class is used as part of a [Point3DKeyFrameCollection](_Point3DKeyFrameCollection.md) in conjunction with a [Point3DAnimationUsingKeyFrames](_Point3DAnimationUsingKeyFrames.md) to animate a Point3D property value along a set of key frames.

A key frame defines a segment of the [Point3DAnimationUsingKeyFrames](_Point3DAnimationUsingKeyFrames.md) to which it belongs. Each key frame has a target Value and a [KeyTime](_KeyTime.md). The [KeyTime](_KeyTime.md) specifies the time at which the key frame's Value should be reached. A key frame animates from the target value of the previous key frame to its own target value. It starts when the previous key frame ends and ends when its own key time is reached.

Spline key frames like [SplinePoint3DKeyFrame](_SplinePoint3DKeyFrame.md) creates a variable transition between values which is determined by the [KeySpline](_KeySpline.md) property. Splined interpolation can be used to achieve more realistic "real world" timing effects such as acceleration and deceleration.
...
| Name | Description |
| --- | --- |
| ● *KeySpline* | Gets or sets the two control Point3Ds that define animation progress for this key frame |
...
SplinePoint3DKeyFrame has no attached properties
...
SplinePoint3DKeyFrame has no events