Source: https://www.noesisengine.com/docs/Gui.Animation._DiscretePoint3DKeyFrame.html

# DiscretePoint3DKeyFrame Class

## namespace [Noesis](../Gui.Core/_ClassHierarchy.md#noesis-namespace) | [MSDN](http://msdn.microsoft.com/en-us/library/system.windows.media.animation.discretepoint3dkeyframe.aspx)

Animates from the *Point3D* value of the previous key frame to its own *Value* using discrete interpolation.

This class is used as part of a [Point3DKeyFrameCollection](_Point3DKeyFrameCollection.md) object in conjunction with a [Point3DAnimationUsingKeyFrames](_Point3DAnimationUsingKeyFrames.md) object to animate a Point3D property value along a set of key frames.
...
Discrete key frames like [DiscretePoint3DKeyFrame](_DiscretePoint3DKeyFrame.md) create sudden "jumps" between values (no interpolation). In other words, the animated property does not change until the key frame's key time is reached, at which point the animated property goes suddenly to the target value.
...
DiscretePoint3DKeyFrame has no attached properties
...
DiscretePoint3DKeyFrame has no events