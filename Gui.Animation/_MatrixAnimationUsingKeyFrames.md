Source: https://www.noesisengine.com/docs/Gui.Animation._MatrixAnimationUsingKeyFrames.html

# MatrixAnimationUsingKeyFrames Class

## namespace [Noesis](../Gui.Core/_ClassHierarchy.md#noesis-namespace) | [MSDN](http://msdn.microsoft.com/en-us/library/system.windows.media.animation.matrixanimationusingkeyframes.aspx)

Animates the value of a *Matrix* property along a set of *KeyFrames*.

A key frame animation's target values are defined by its KeyFrames property, which contains a collection of [MatrixKeyFrame](_MatrixKeyFrame.md) objects. Each [MatrixKeyFrame](_MatrixKeyFrame.md) defines a segment of the animation with its own target Value and [KeyTime](_KeyTime.md). When the animation runs, it progresses from one key value to the next at the specified key times.
...
## From [MatrixAnimationBase](_MatrixAnimationBase.md)
