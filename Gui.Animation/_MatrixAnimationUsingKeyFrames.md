...
A key frame animation's target values are defined by its KeyFrames property, which contains a collection of [MatrixKeyFrame](/Gui.Animation/_MatrixKeyFrame.md) objects. Each [MatrixKeyFrame](/Gui.Animation/_MatrixKeyFrame.md) defines a segment of the animation with its own target Value and [KeyTime](/Gui.Animation/_KeyTime.md). When the animation runs, it progresses from one key value to the next at the specified key times.
...
## From [MatrixAnimationBase](/Gui.Animation/_MatrixAnimationBase.md)
