...
A key frame animation's target values are defined by its KeyFrames property, which contains a collection of [ColorKeyFrame](_ColorKeyFrame.md) objects. Each [ColorKeyFrame](_ColorKeyFrame.md) defines a segment of the animation with its own target Value and [KeyTime](_KeyTime.md). When the animation runs, it progresses from one key value to the next at the specified key times.

There are four types of [ColorKeyFrame](_ColorKeyFrame.md) classes, one for each supported interpolation method: [LinearColorKeyFrame](_LinearColorKeyFrame.md), [DiscreteColorKeyFrame](_DiscreteColorKeyFrame.md), [EasingColorKeyFrame](_EasingColorKeyFrame.md), and [SplineColorKeyFrame](_SplineColorKeyFrame.md).

Unlike a [ColorAnimation](_ColorAnimation.md), a [ColorAnimationUsingKeyFrames](_ColorAnimationUsingKeyFrames.md) can have more than two of target values. You can also control the interpolation method of individual [ColorKeyFrame](_ColorKeyFrame.md) segments.
...
• [ColorAnimationBase](_ColorAnimationBase.md)
...
## From [ColorAnimationBase](_ColorAnimationBase.md)
