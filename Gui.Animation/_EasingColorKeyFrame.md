...
A class that enables you to associate easing functions with a [ColorAnimationUsingKeyFrames](_ColorAnimationUsingKeyFrames.md) key frame animation.

For an example of using an object similar to this one, see [EasingDoubleKeyFrame](_EasingDoubleKeyFrame.md).

This class is used as part of a [ColorKeyFrameCollection](_ColorKeyFrameCollection.md) in conjunction with a [ColorAnimationUsingKeyFrames](_ColorAnimationUsingKeyFrames.md) to animate a Color property value along a set of key frames. This [ColorKeyFrame](_ColorKeyFrame.md) interpolates between the Color value of the previous key frame and its own value linearly with an easing function to produce its output value.
