Source: https://www.noesisengine.com/docs/Gui.Animation._PointAnimationUsingKeyFrames.html

# PointAnimationUsingKeyFrames Class

## namespace [Noesis](/Gui.Core/_ClassHierarchy.md#noesis-namespace) | [MSDN](http://msdn.microsoft.com/en-us/library/system.windows.media.animation.pointanimationusingkeyframes.aspx)

Animates the value of a *Point* property along a set of *KeyFrames*.

A key frame animation's target values are defined by its KeyFrames property, which contains a collection of [PointKeyFrame](/Gui.Animation/_PointKeyFrame.md) objects. Each [PointKeyFrame](/Gui.Animation/_PointKeyFrame.md) defines a segment of the animation with its own target Value and [KeyTime](/Gui.Animation/_KeyTime.md). When the animation runs, it progresses from one key value to the next at the specified key times.

There are three types of [PointKeyFrame](/Gui.Animation/_PointKeyFrame.md) classes, one for each supported interpolation method: [LinearPointKeyFrame](/Gui.Animation/_LinearPointKeyFrame.md), [DiscretePointKeyFrame](/Gui.Animation/_DiscretePointKeyFrame.md), and [SplinePointKeyFrame](/Gui.Animation/_SplinePointKeyFrame.md).

Unlike a [PointAnimation](/Gui.Animation/_PointAnimation.md), a [PointAnimationUsingKeyFrames](/Gui.Animation/_PointAnimationUsingKeyFrames.md) can have more than two of target values. You can also control the interpolation method of individual [PointKeyFrame](/Gui.Animation/_PointKeyFrame.md) segments.
...
• [PointAnimationBase](/Gui.Animation/_PointAnimationBase.md)
...
## From [PointAnimationBase](/Gui.Animation/_PointAnimationBase.md)
