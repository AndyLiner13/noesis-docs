...
A key frame animation's target values are defined by its KeyFrames property, which contains a collection of [PointKeyFrame](_PointKeyFrame.md) objects. Each [PointKeyFrame](_PointKeyFrame.md) defines a segment of the animation with its own target Value and [KeyTime](_KeyTime.md). When the animation runs, it progresses from one key value to the next at the specified key times.

There are three types of [PointKeyFrame](_PointKeyFrame.md) classes, one for each supported interpolation method: [LinearPointKeyFrame](_LinearPointKeyFrame.md), [DiscretePointKeyFrame](_DiscretePointKeyFrame.md), and [SplinePointKeyFrame](_SplinePointKeyFrame.md).

Unlike a [PointAnimation](_PointAnimation.md), a [PointAnimationUsingKeyFrames](_PointAnimationUsingKeyFrames.md) can have more than two of target values. You can also control the interpolation method of individual [PointKeyFrame](_PointKeyFrame.md) segments.
...
• [PointAnimationBase](_PointAnimationBase.md)
...
## From [PointAnimationBase](_PointAnimationBase.md)
