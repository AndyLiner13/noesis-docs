...
Contains mutually exclusive [VisualState](_VisualState.md) objects and [VisualTransition](_VisualTransition.md) objects that are used to go from one state to another.
...
| Name | Description |
| --- | --- |
| ○ *Name* | Gets the name of the [VisualStateGroup](_VisualStateGroup.md). |
| ○ *States* | Gets the collection of mutually exclusive [VisualState](_VisualState.md) objects |
| ○ *Transitions* | Gets the collection of [VisualTransition](_VisualTransition.md) objects |
...
| Name | Description |
| --- | --- |
|  *CreateTransitionStoryboard(root, newState, transition)* | Creates a [Storyboard](_Storyboard.md) that animates changes from current state to the specifed new state |
|  *FindState(name)* | Finds a matching state with the same name (can be null or empty) |
|  *FindTransition(from, to)* | Finds a matching transition for the provided *from* and "to\* [VisualState](_VisualState.md) objects |
