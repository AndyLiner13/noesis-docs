Source: https://www.noesisengine.com/docs/Gui.Animation._RectAnimationUsingKeyFrames.html

# RectAnimationUsingKeyFrames Class

## namespace [Noesis](/Gui.Core/_ClassHierarchy.md#noesis-namespace) | [MSDN](http://msdn.microsoft.com/en-us/library/system.windows.media.animation.rectanimationusingkeyframes.aspx)

Animates the value of a *Rect* property along a set of *KeyFrames*.

A key frame animation's target values are defined by its KeyFrames property, which contains a collection of [RectKeyFrame](/Gui.Animation/_RectKeyFrame.md) objects. Each [RectKeyFrame](/Gui.Animation/_RectKeyFrame.md) defines a segment of the animation with its own target Value and [KeyTime](/Gui.Animation/_KeyTime.md). When the animation runs, it progresses from one key value to the next at the specified key times.

There are three types of [RectKeyFrame](/Gui.Animation/_RectKeyFrame.md) classes, one for each supported interpolation method: [LinearRectKeyFrame](/Gui.Animation/_LinearRectKeyFrame.md), [DiscreteRectKeyFrame](/Gui.Animation/_DiscreteRectKeyFrame.md), and [SplineRectKeyFrame](/Gui.Animation/_SplineRectKeyFrame.md).

Unlike a [RectAnimation](/Gui.Animation/_RectAnimation.md), a [RectAnimationUsingKeyFrames](/Gui.Animation/_RectAnimationUsingKeyFrames.md) can have more than two target values. You can also control the interpolation method of individual [RectKeyFrame](/Gui.Animation/_RectKeyFrame.md) segments.
...
• [RectAnimationBase](/Gui.Animation/_RectAnimationBase.md)
...
## From [RectAnimationBase](/Gui.Animation/_RectAnimationBase.md)
