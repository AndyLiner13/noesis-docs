...
A key frame animation's target values are defined by its KeyFrames property, which contains a collection of [RectKeyFrame](_RectKeyFrame.md) objects. Each [RectKeyFrame](_RectKeyFrame.md) defines a segment of the animation with its own target Value and [KeyTime](_KeyTime.md). When the animation runs, it progresses from one key value to the next at the specified key times.

There are three types of [RectKeyFrame](_RectKeyFrame.md) classes, one for each supported interpolation method: [LinearRectKeyFrame](_LinearRectKeyFrame.md), [DiscreteRectKeyFrame](_DiscreteRectKeyFrame.md), and [SplineRectKeyFrame](_SplineRectKeyFrame.md).

Unlike a [RectAnimation](_RectAnimation.md), a [RectAnimationUsingKeyFrames](_RectAnimationUsingKeyFrames.md) can have more than two target values. You can also control the interpolation method of individual [RectKeyFrame](_RectKeyFrame.md) segments.
...
• [RectAnimationBase](_RectAnimationBase.md)
...
## From [RectAnimationBase](_RectAnimationBase.md)
