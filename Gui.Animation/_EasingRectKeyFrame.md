...
A class that enables you to associate easing functions with a [RectAnimationUsingKeyFrames](_RectAnimationUsingKeyFrames.md) key frame animation.

For an example of using an object similar to this one, see [EasingDoubleKeyFrame](_EasingDoubleKeyFrame.md).

This class is used as part of a [RectKeyFrameCollection](_RectKeyFrameCollection.md) in conjunction with a [RectAnimationUsingKeyFrames](_RectAnimationUsingKeyFrames.md) to animate a Rect property value along a set of key frames. This [RectKeyFrame](_RectKeyFrame.md) interpolates between the Rect value of the previous key frame and its own value linearly with an easing function to produce its output value.
