Source: https://www.noesisengine.com/docs/Gui.Animation._DiscreteInt64KeyFrame.html

# DiscreteInt64KeyFrame Class

## namespace [Noesis](../Gui.Core/_ClassHierarchy.md#noesis-namespace) | [MSDN](http://msdn.microsoft.com/en-us/library/system.windows.media.animation.discreteint64keyframe.aspx)

Animates from the *Int64* value of the previous key frame to its own *Value* using discrete interpolation.

This class is used as part of a [Int64KeyFrameCollection](_Int64KeyFrameCollection.md) in conjunction with a [Int64AnimationUsingKeyFrames](_Int64AnimationUsingKeyFrames.md) to animate a Int64 property value along a set of key frames.

A key frame defines a segment of the [Int64AnimationUsingKeyFrames](_Int64AnimationUsingKeyFrames.md) to which it belongs. Each key frame has a target Value and a [KeyTime](_KeyTime.md). The [KeyTime](_KeyTime.md) specifies the time at which the key frame's Value should be reached. A key frame animates from the target value of the previous key frame to its own target value. It starts when the previous key frame ends and ends when its own key time is reached.

Discrete key frames like [DiscreteInt64KeyFrame](_DiscreteInt64KeyFrame.md) create sudden "jumps" between values (no Interpolation). In other words, the animated property does not change until the key frame's key time is reached at which point the animated property goes suddenly to the target value.
...
• [Int64KeyFrame](_Int64KeyFrame.md)

• *DiscreteInt64KeyFrame*

# Properties

## From [Int64KeyFrame](_Int64KeyFrame.md)
...
DiscreteInt64KeyFrame has no attached properties
...
DiscreteInt64KeyFrame has no events