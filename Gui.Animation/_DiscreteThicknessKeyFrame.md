Source: https://www.noesisengine.com/docs/Gui.Animation._DiscreteThicknessKeyFrame.html

# DiscreteThicknessKeyFrame Class

## namespace [Noesis](../Gui.Core/_ClassHierarchy.md#noesis-namespace) | [MSDN](http://msdn.microsoft.com/en-us/library/system.windows.media.animation.discretethicknesskeyframe.aspx)

Animates from the *Thickness* value of the previous key frame to its own *Value* using discrete interpolation.

his class is used as part of a [ThicknessKeyFrameCollection](_ThicknessKeyFrameCollection.md) in conjunction with a [ThicknessAnimationUsingKeyFrames](_ThicknessAnimationUsingKeyFrames.md) to animate a Thickness property value along a set of key frames.

A key frame defines a segment of the [ThicknessAnimationUsingKeyFrames](_ThicknessAnimationUsingKeyFrames.md) to which it belongs. Each key frame has a target Value and a [KeyTime](_KeyTime.md). The [KeyTime](_KeyTime.md) specifies the time at which the key frame's Value should be reached. A key frame animates from the target value of the previous key frame to its own target value. It starts when the previous key frame ends and ends when its own key time is reached.

Discrete key frames like [DiscreteThicknessKeyFrame](_DiscreteThicknessKeyFrame.md) create sudden "jumps" between values (no interpolation). In other words, the animated property does not change until the key frame's key time is reached, at which point the animated property goes suddenly to the target value.
...
• [ThicknessKeyFrame](_ThicknessKeyFrame.md)

• *DiscreteThicknessKeyFrame*

# Properties

## From [ThicknessKeyFrame](_ThicknessKeyFrame.md)
...
DiscreteThicknessKeyFrame has no attached properties
...
DiscreteThicknessKeyFrame has no events