...
This class is used as part of a [ColorKeyFrameCollection](/Gui.Animation/_ColorKeyFrameCollection.md) in conjunction with a [ColorAnimationUsingKeyFrames](/Gui.Animation/_ColorAnimationUsingKeyFrames.md) to animate a Color property value along a set of key frames.

A key frame defines a segment of the [ColorAnimationUsingKeyFrames](/Gui.Animation/_ColorAnimationUsingKeyFrames.md) to which it belongs. Each key frame has a target Value and a [KeyTime](/Gui.Animation/_KeyTime.md). The [KeyTime](/Gui.Animation/_KeyTime.md) specifies the time at which the key frame's Value should be reached. A key frame animates from the target value of the previous key frame to its own target value. It starts when the previous key frame ends and ends when its own key time is reached.
