Source: https://www.noesisengine.com/docs/Gui.Animation._SplineColorKeyFrame.html

# SplineColorKeyFrame Class

## namespace [Noesis](../Gui.Core/_ClassHierarchy.md#noesis-namespace) | [MSDN](http://msdn.microsoft.com/en-us/library/system.windows.media.animation.splinecolorkeyframe.aspx)

Animates from the *Color* value of the previous key frame to its own *Value* using splined interpolation.

This class is used as part of a [ColorKeyFrameCollection](_ColorKeyFrameCollection.md) in conjunction with a [ColorAnimationUsingKeyFrames](_ColorAnimationUsingKeyFrames.md) to animate a Color property value along a set of key frames.
...
Spline key frames like [SplineColorKeyFrame](_SplineColorKeyFrame.md) creates a variable transition between values which is determined by the [KeySpline](_KeySpline.md) property. Splined interpolation can be used to achieve more realistic "real world" timing effects such as acceleration and deceleration.
...
SplineColorKeyFrame has no attached properties
...
SplineColorKeyFrame has no events