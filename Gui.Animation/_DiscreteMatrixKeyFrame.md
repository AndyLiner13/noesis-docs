Source: https://www.noesisengine.com/docs/Gui.Animation._DiscreteMatrixKeyFrame.html

# DiscreteMatrixKeyFrame Class

## namespace [Noesis](../Gui.Core/_ClassHierarchy.md#noesis-namespace) | [MSDN](http://msdn.microsoft.com/en-us/library/system.windows.media.animation.discretematrixkeyframe.aspx)

Animates from the *Matrix* value of the previous key frame to its own *Value* using discrete interpolation.

This class is used as part of a [MatrixKeyFrameCollection](_MatrixKeyFrameCollection.md) in conjunction with a [MatrixAnimationUsingKeyFrames](_MatrixAnimationUsingKeyFrames.md) to animate a Matrix property value along a set of key frames.

A key frame defines a segment of the [MatrixAnimationUsingKeyFrames](_MatrixAnimationUsingKeyFrames.md) to which it belongs. Each key frame has a target Value and a [KeyTime](_KeyTime.md). The [KeyTime](_KeyTime.md) specifies the time at which the key frame's Value should be reached. A key frame animates from the target value of the previous key frame to its own target value. It starts when the previous key frame ends and ends when its own key time is reached.

Discrete key frames like [DiscreteMatrixKeyFrame](_DiscreteMatrixKeyFrame.md) create sudden "jumps" between values (no Interpolation). In other words, the animated property does not change until the key frame's key time is reached at which point the animated property goes suddenly to the target value.
...
• [MatrixKeyFrame](_MatrixKeyFrame.md)

• *DiscreteMatrixKeyFrame*

# Properties

## From [MatrixKeyFrame](_MatrixKeyFrame.md)
...
DiscreteMatrixKeyFrame has no attached properties
...
DiscreteMatrixKeyFrame has no events