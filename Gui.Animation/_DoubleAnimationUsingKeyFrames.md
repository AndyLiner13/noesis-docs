...
A key frame animation's target values are defined by its KeyFrames property, which contains a collection of [DoubleKeyFrame](_DoubleKeyFrame.md) objects. Each [DoubleKeyFrame](_DoubleKeyFrame.md) defines a segment of the animation with its own target Value and [KeyTime](_KeyTime.md). When the animation runs, it progresses from one key value to the next at the specified key times.

There are three types of [DoubleKeyFrame](_DoubleKeyFrame.md) classes, one for each supported interpolation method: [LinearDoubleKeyFrame](_LinearDoubleKeyFrame.md), [DiscreteDoubleKeyFrame](_DiscreteDoubleKeyFrame.md), and [SplineDoubleKeyFrame](_SplineDoubleKeyFrame.md).

Unlike a [DoubleAnimation](_DoubleAnimation.md), a [DoubleAnimationUsingKeyFrames](_DoubleAnimationUsingKeyFrames.md) can have more than two of target values. You can also control the interpolation method of individual [DoubleKeyFrame](_DoubleKeyFrame.md) segments.
...
• [DoubleAnimationBase](_DoubleAnimationBase.md)
...
## From [DoubleAnimationBase](_DoubleAnimationBase.md)
