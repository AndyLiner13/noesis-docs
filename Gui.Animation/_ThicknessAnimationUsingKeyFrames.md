...
A key frame animation's target values are defined by its KeyFrames property, which contains a collection of [ThicknessKeyFrame](_ThicknessKeyFrame.md) objects. Each [ThicknessKeyFrame](_ThicknessKeyFrame.md) defines a segment of the animation with its own target Value and [KeyTime](_KeyTime.md). When the animation runs, it progresses from one key value to the next at the specified key times.

There are three types of [ThicknessKeyFrame](_ThicknessKeyFrame.md) classes, one for each supported interpolation method: [LinearThicknessKeyFrame](_LinearThicknessKeyFrame.md), [DiscreteThicknessKeyFrame](_DiscreteThicknessKeyFrame.md), and [SplineThicknessKeyFrame](_SplineThicknessKeyFrame.md).

Unlike a [ThicknessAnimation](_ThicknessAnimation.md), a [ThicknessAnimationUsingKeyFrames](_ThicknessAnimationUsingKeyFrames.md) can have more than two of target values. You can also control the interpolation method of individual [ThicknessKeyFrame](_ThicknessKeyFrame.md) segments.
...
• [ThicknessAnimationBase](_ThicknessAnimationBase.md)
...
## From [ThicknessAnimationBase](_ThicknessAnimationBase.md)
