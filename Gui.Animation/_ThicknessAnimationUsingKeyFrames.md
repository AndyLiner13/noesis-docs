Source: https://www.noesisengine.com/docs/Gui.Animation._ThicknessAnimationUsingKeyFrames.html

# ThicknessAnimationUsingKeyFrames Class

## namespace [Noesis](/Gui.Core/_ClassHierarchy.md#noesis-namespace) | [MSDN](http://msdn.microsoft.com/en-us/library/system.windows.media.animation.thicknessanimationusingkeyframes.aspx)

Animates the value of a *Thickness* property along a set of *KeyFrames*.

A key frame animation's target values are defined by its KeyFrames property, which contains a collection of [ThicknessKeyFrame](/Gui.Animation/_ThicknessKeyFrame.md) objects. Each [ThicknessKeyFrame](/Gui.Animation/_ThicknessKeyFrame.md) defines a segment of the animation with its own target Value and [KeyTime](/Gui.Animation/_KeyTime.md). When the animation runs, it progresses from one key value to the next at the specified key times.

There are three types of [ThicknessKeyFrame](/Gui.Animation/_ThicknessKeyFrame.md) classes, one for each supported interpolation method: [LinearThicknessKeyFrame](/Gui.Animation/_LinearThicknessKeyFrame.md), [DiscreteThicknessKeyFrame](/Gui.Animation/_DiscreteThicknessKeyFrame.md), and [SplineThicknessKeyFrame](/Gui.Animation/_SplineThicknessKeyFrame.md).

Unlike a [ThicknessAnimation](/Gui.Animation/_ThicknessAnimation.md), a [ThicknessAnimationUsingKeyFrames](/Gui.Animation/_ThicknessAnimationUsingKeyFrames.md) can have more than two of target values. You can also control the interpolation method of individual [ThicknessKeyFrame](/Gui.Animation/_ThicknessKeyFrame.md) segments.
...
• [ThicknessAnimationBase](/Gui.Animation/_ThicknessAnimationBase.md)
...
## From [ThicknessAnimationBase](/Gui.Animation/_ThicknessAnimationBase.md)
