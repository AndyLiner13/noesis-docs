...
This class is used as part of a [ObjectKeyFrameCollection](_ObjectKeyFrameCollection.md) in conjunction with a [ObjectAnimationUsingKeyFrames](_ObjectAnimationUsingKeyFrames.md) to animate a Object property value along a set of key frames.

A key frame defines a segment of the [ObjectAnimationUsingKeyFrames](_ObjectAnimationUsingKeyFrames.md) to which it belongs. Each key frame has a target Value and a [KeyTime](_KeyTime.md). The [KeyTime](_KeyTime.md) specifies the time at which the key frame's Value should be reached. A key frame animates from the target value of the previous key frame to its own target value. It starts when the previous key frame ends and ends when its own key time is reached.

Discrete key frames like [DiscreteObjectKeyFrame](_DiscreteObjectKeyFrame.md) create sudden "jumps" between values (no Interpolation). In other words, the animated property does not change until the key frame's key time is reached at which point the animated property goes suddenly to the target value.
...
• [ObjectKeyFrame](_ObjectKeyFrame.md)

• *DiscreteObjectKeyFrame*

# Properties

## From [ObjectKeyFrame](_ObjectKeyFrame.md)
