Source: https://www.noesisengine.com/docs/Gui.Animation._DiscreteSizeKeyFrame.html

# DiscreteSizeKeyFrame Class

## namespace [Noesis](../Gui.Core/_ClassHierarchy.md#noesis-namespace) | [MSDN](http://msdn.microsoft.com/en-us/library/system.windows.media.animation.discretesizekeyframe.aspx)

Animates from the *Size* value of the previous key frame to its own *Value* using discrete interpolation.

This class is used as part of a [SizeKeyFrameCollection](_SizeKeyFrameCollection.md) in conjunction with a [SizeAnimationUsingKeyFrames](_SizeAnimationUsingKeyFrames.md) to animate a Size property value along a set of key frames.

A key frame defines a segment of the [SizeAnimationUsingKeyFrames](_SizeAnimationUsingKeyFrames.md) to which it belongs. Each key frame has a target Value and a [KeyTime](_KeyTime.md). The [KeyTime](_KeyTime.md) specifies the time at which the key frame's Value should be reached. A key frame animates from the target value of the previous key frame to its own target value. It starts when the previous key frame ends and ends when its own key time is reached.

Discrete key frames like [DiscreteSizeKeyFrame](_DiscreteSizeKeyFrame.md) create sudden "jumps" between values (no Interpolation). In other words, the animated property does not change until the key frame's key time is reached at which point the animated property goes suddenly to the target value.
...
• [SizeKeyFrame](_SizeKeyFrame.md)

• *DiscreteSizeKeyFrame*

# Properties

## From [SizeKeyFrame](_SizeKeyFrame.md)
...
DiscreteSizeKeyFrame has no attached properties
...
DiscreteSizeKeyFrame has no events