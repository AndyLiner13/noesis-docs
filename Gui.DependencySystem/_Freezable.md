...
Classes that derive from [Freezable](/Gui.DependencySystem/_Freezable.md) provide detailed change notification, can be made immutable, and can clone themselves. The [Freezable](/Gui.DependencySystem/_Freezable.md) class provides special features that can help improve application performance when using objects that are expensive to modify or copy.

A class that derives from [Freezable](/Gui.DependencySystem/_Freezable.md) gains the following features:

- Special states: a read-only (frozen) state and a writable state.
- Thread safety: a frozen [Freezable](/Gui.DependencySystem/_Freezable.md) object can be shared across threads.
- Detailed change notification: Unlike other [DependencyObject](/Gui.DependencySystem/_DependencyObject.md) objects, a [Freezable](/Gui.DependencySystem/_Freezable.md) object
  provides change notifications when sub-property values change
- Easy cloning: the [Freezable](/Gui.DependencySystem/_Freezable.md) class has already implemented several methods that produce
  deep clones
