Source: https://www.noesisengine.com/docs/App.Interactivity._GamepadTrigger.html

# GamepadTrigger Class

## namespace [NoesisApp](../Gui.Core/_ClassHierarchy.md#noesisapp-namespace)

A [Trigger](../Gui.Core/_Trigger.md) that is triggered by a specified gamepad button.
...
• [TriggerBase](_TriggerBase.md)

• [TriggerBaseT](_TriggerBaseT.md)
...
| Name | Description |
| --- | --- |
| ● *ActiveOnFocus* | If true, the [Trigger](../Gui.Core/_Trigger.md) listens to *ButtonDown* or *ButtonUp* events raised by its *Source* object, which means that element must have focus for the trigger to fire. If false, the [Trigger](../Gui.Core/_Trigger.md) listens at the root, so any unhandled *ButtonDown* and *ButtonUp* messages will be caught |
| ● *Button* | The button that must be pressed or released for the trigger to fire |
| ● *FiredOn* | Determines if trigger happens when gamepad button is pressed or released |
| ● *HandleWhenFired* | Indicates if [GamepadTrigger](_GamepadTrigger.md) should set gamepad button event as handled when the trigger invokes its actions |

● Dependency Property   ○ Reflection Property

## From [TriggerBase](_TriggerBase.md)
...
GamepadTrigger has no attached properties
