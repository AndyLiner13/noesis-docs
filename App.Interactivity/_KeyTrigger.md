Source: https://www.noesisengine.com/docs/App.Interactivity._KeyTrigger.html

# KeyTrigger Class

## namespace [NoesisApp](../Gui.Core/_ClassHierarchy.md#noesisapp-namespace)

A [Trigger](../Gui.Core/_Trigger.md) that is triggered by a keyboard event. If *Key* and *Modifiers* are detected, it fires.
...
| Name | Description |
| --- | --- |
| ● *ActiveOnFocus* | If true, the [Trigger](../Gui.Core/_Trigger.md) only listens to its trigger Source object, which means that element must have focus for the trigger to fire. If false, the [Trigger](../Gui.Core/_Trigger.md) listens at the root, so any unhandled KeyDown/Up messages will be caught |
...
| ● *Modifiers* | The modifiers that must be active for the trigger to fire |
...
KeyTrigger has no attached properties
