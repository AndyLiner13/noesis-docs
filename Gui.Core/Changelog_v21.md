...
- Fixed [RadioButton](/Gui.Core/_RadioButton.md) was incorrectly setting *IsChecked* local value on click.
...
- Fixed Crash if no [Panel](/Gui.Core/_Panel.md) was set as root of *ItemsPanelTemplate*.
...
- Enhancement New [Integration](/Gui.Core/SDKGuide.md) tutorial.
...
- Enhancement [RepeatButton](/Gui.Core/_RepeatButton.md) default values for *Delay*, *Interval* and *ClickMode* matching WPF.
...
- Enhancement C++ *FindResource(const char\*)* added to [FrameworkElement](/Gui.Core/_FrameworkElement.md).
...
- Fixed [CheckBox](/Gui.Core/_CheckBox.md) default value for *KeyboardNavigation.AcceptsReturn* matching WPF.
...
- Enhancement [Blend](/Gui.Core/BlendTutorial.md), [XamlPlayer](/Gui.Core/FirstSteps.md) and [Performance](/Gui.Core/Optimizing.md) tutorials updated.
...
- Enhancement [Styling tutorial](/Gui.Core/StylingTutorial.md) updated. Global dictionary now behaves exactly as *WPF* *Application.Resources*.
...
- Fixed Artifacts using [VisualBrush](/Gui.Core/_VisualBrush.md) with *PPA* enabled.

# Version 2.1.0b15

- Enhancement New [C++ Architecture Guide](/Gui.Core/CppArchitectureGuide.md).
...
- Fixed [ContentPresenter](/Gui.Core/_ContentPresenter.md) not disconnecting template triggers when root was [TextBlock](/Gui.Core/_TextBlock.md).
- Fixed *Items* not disconnected from [ContentPresenter](/Gui.Core/_ContentPresenter.md) when directly connected without *DataTemplate*.
- Fixed [TabControl](/Gui.Core/_TabControl.md) can move now between tabs using *Ctrl+Tab*.
- Fixed [ComboBox](/Gui.Core/_ComboBox.md) and [Slider](/Gui.Core/_Slider.md) fully working with keyboard.
...
- Enhancement [KeyBinding](/Gui.Core/_KeyBinding.md) exposing *Key* and *Modifiers* as bindable dependency properties.
...
- Enhancement Implemented [BaseTextBox](/Gui.Core/_BaseTextBox.md) *TextChanged* and *SelectionChanged* events.
...
- Fixed [Button](/Gui.Core/_Button.md) now overrides *AcceptsReturn* to raise *Click* event on return key pressed.
- Fixed Float rounding in layout generating unnecessary clipping.
- Fixed [TabControl](/Gui.Core/_TabControl.md) not displaying tab content if *SelectedIndex* was initially set.
- Fixed Crash when hovering over an element already destroyed.
- Fixed Horizontal scroll of [ListView](/Gui.Controls/_ListView.md) / [ListBox](/Gui.Core/_ListBox.md) not working.
- Fixed [ContentPresenter](/Gui.Core/_ContentPresenter.md) was not removing old parent in some cases.
...
- Fixed [TextBox](/Gui.Core/_TextBox.md) internal text element shouldn't be affected by TextBlock styles.
...
- Enhancement [Button](/Gui.Core/_Button.md) and [MenuItem](/Gui.Core/_MenuItem.md) *IsEnabled* property updated when command *CanExecuteChanged* raised.
...
- Fixed Crash with binding set in non [FrameworkElement](/Gui.Core/_FrameworkElement.md) object.
...
- Fixed [ListBox](/Gui.Core/_ListBox.md) clipping not updated when using virtualization and changing size.
- Fixed Selection not updated when adding or removing items to source collection.
- Fixed Right clicking an item does not select it when [ContextMenu](/Gui.Core/_ContextMenu.md) is open.
- Fixed Mouse Down/Up events order now matches WPF order.
- Fixed *TextBlock.Run* not receiving *ViewModel* updates.
- Fixed [ComboBox](/Gui.Core/_ComboBox.md) appears with no selection even with *SelectedIndex* bound.
- Fixed [TextBlock](/Gui.Core/_TextBlock.md) and other classes didn't override *DefaultStyleKey* property.
- Fixed *EventTriggers* in theme style not working.
- Fixed Crash on [TextBlock](/Gui.Core/_TextBlock.md) mouse over when inlines dynamically added.
- Fixed Ribbon UI buttons not working first time selection is made.
- Fixed [UIElement](/Gui.Core/_UIElement.md) flag properties still with old values when *PropertyChangedCallback* invoked.
...
- Fixed [Trigger](/Gui.Core/_Trigger.md) actions in value providers invoked in inverse order of priority.
- Fixed Crash in binding with *RelativeSource* *FindAncestor* when type was not resolved.
- Fixed [Popup](/Gui.Core/_Popup.md) *Closed* event was raised twice for [ContextMenu](/Gui.Core/_ContextMenu.md).
...
- Fixed Bindings not working sometimes in [TextBlock](/Gui.Core/_TextBlock.md) inlines when using DataContext as source.
- Fixed *Binding.ElementName* doesn't work in a DataTrigger/MultiDataTrigger if it refers to template child.
- Fixed Unable to register named [UserControl](/Gui.Core/_UserControl.md) in a template if using the same name for the root in the associated XAML.
...
- Fixed [ListBox](/Gui.Core/_ListBox.md) moves items left when pressing buttons.