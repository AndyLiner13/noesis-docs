...
![WorkflowTutorialImg1.png](/WorkflowTutorialImg1/png.md)
...
In Noesis, the *View* contains the UI elements users interact with. Views are updated and rendered per frame. More information is available in the [Rendering Tutorial](/Gui.Core/RenderingTutorial.md). Following [MVVM](https://en.wikipedia.org/wiki/Model%E2%80%93view%E2%80%93viewmodel) naming conventions, *View* is also referred in this tutorial as the XAML that designers create using tools like *Blend*.

![WorkflowTutorialImg7.png](/WorkflowTutorialImg7/png.md)

The *View* gets the information from the *DataContext* using [Data Binding](/Gui.Core/DataBindingTutorial.md). We always recommend this pure *MVVM* approach when working with Noesis. This way roles are very clear when working on the UI:

- **Programmers** expose the available information through the *DataContext*, this is the *Model* in MVVM parlance. The *DataContext* is a hierarchical container of information. Note that the *DataContext* not only contains values like floats or integers. It can also expose complex objects like collections or commands. With [Commands](/Gui.Core/CommandsTutorial.md), designers can invoke functions from the XAML. The *DataContext* can also expose [Events](/App.Interactivity/_DataEventTrigger.md) that can be used in XAML as triggers to execute [Interactivity](/App.Interactivity/Behaviors.md) actions.
...
Unreal will automatically reload XAML changes, even in *Play* mode. Make sure the *Blend* project is properly mapped as explained in the [Integration guide](/Gui.Core/UnrealTutorial.md).

![WorkflowTutorialImg2.png](/WorkflowTutorialImg2/png.md)
...
By default, Unity will automatically reload XAML changes while in *Play* mode. Find more information in the [Integration guide](/Gui.Core/Unity3DTutorial.md).

![WorkflowTutorialImg3.png](/WorkflowTutorialImg3/png.md)

## Custom Engine

If the custom engine is using the C++ SDK, then the *DataContext* can be created using the [reflection](/Gui.Core/CppArchitectureGuide.md#reflection) implemented in Noesis. C# already provides a reflection natively implemented in the language. In both cases, this needs the assistance of programers so we recommend using **Design-Time** *DataContexts* with Blend (more on this later).

To support hot reloading in proprietary engines, custom [Providers](/Gui.Core/ProvidersTutorial.md#hot-reloading) or the [Inspector](/Gui.Core/InspectorTutorial.md) tool must be used.

# Microsoft Blend

[Blend](/Gui.Core/BlendTutorial.md) is the tool we always recommend for editing XAML. Assets like *XAMLs*, *Images* and *Fonts* must be located in a folder shared with the runtime (Unreal, Unity, Custom). That way, resources are not duplicated and each time they are edited in Blend, they hot reload in the application. With this setup, and for example using multiple monitors, you can have Blend in one monitor and Unity or Unreal in a second one. Thanks to [Hot Reloading](/Gui.Core/HotReloadTutorial.md), the engine acts like a WYSIWYG editor of *Blend* as changes are applied instantly.
...
![WorkflowTutorialImg6.png](/WorkflowTutorialImg6/png.md)
...
![WorkflowTutorialImg4.png](/WorkflowTutorialImg4/png.md)
...
![WorkflowTutorialImg5.png](/WorkflowTutorialImg5/png.md)
...
Development versions of Noesis, supports remote connections from the [Inspector](/Gui.Core/InspectorTutorial.md). One of the key features provided by the *Inspector* is live editing of objects in the visual tree. The *Inspector* is supported and enabled by default in Unity, Unreal and custom engines.

![WorkflowTutorialImg8.png](/WorkflowTutorialImg8/png.md)