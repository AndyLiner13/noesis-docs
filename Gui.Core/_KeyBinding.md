...
## namespace [Noesis](_ClassHierarchy.md#noesis-namespace) | [MSDN](http://msdn.microsoft.com/en-us/library/system.windows.input.keybinding.aspx)

Binds a [KeyGesture](_KeyGesture.md) to a Command.

When defining a [KeyBinding](_KeyBinding.md) in XAML there are two ways to specify the [KeyGesture](_KeyGesture.md). The first way to establish a [KeyBinding](_KeyBinding.md) in XAML is to define the Gesture attribute of the [KeyBinding](_KeyBinding.md) element, which enables a syntax to specify keys and modifiers as a single string, for example "CTRL+P". The second way is to define the Key attribute and the Modifiers attributes of the [KeyBinding](_KeyBinding.md) element. Both ways of setting the [KeyGesture](_KeyGesture.md) are equivalent and modify the same underlying object, but there will be a conflict if both are used.
...
| Name | Description |
| --- | --- |
| ● *Key* | Gets or sets the Key of the [KeyGesture](_KeyGesture.md) associated with this [KeyBinding](_KeyBinding.md) |
| ● *Modifiers* | Gets the modifier keys (one or more Alt, Ctrl, Shift) of the associated [KeyGesture](_KeyGesture.md) |

● Dependency Property   ○ Reflection Property

## From [InputBinding](_InputBinding.md)
