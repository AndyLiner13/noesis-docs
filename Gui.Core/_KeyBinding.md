Source: https://www.noesisengine.com/docs/Gui.Core._KeyBinding.html

# KeyBinding Class

## namespace [Noesis](/Gui.Core/_ClassHierarchy.md#noesis-namespace) | [MSDN](http://msdn.microsoft.com/en-us/library/system.windows.input.keybinding.aspx)

Binds a [KeyGesture](/Gui.Core/_KeyGesture.md) to a Command.

When defining a [KeyBinding](/Gui.Core/_KeyBinding.md) in XAML there are two ways to specify the [KeyGesture](/Gui.Core/_KeyGesture.md). The first way to establish a [KeyBinding](/Gui.Core/_KeyBinding.md) in XAML is to define the Gesture attribute of the [KeyBinding](/Gui.Core/_KeyBinding.md) element, which enables a syntax to specify keys and modifiers as a single string, for example "CTRL+P". The second way is to define the Key attribute and the Modifiers attributes of the [KeyBinding](/Gui.Core/_KeyBinding.md) element. Both ways of setting the [KeyGesture](/Gui.Core/_KeyGesture.md) are equivalent and modify the same underlying object, but there will be a conflict if both are used.
...
| Name | Description |
| --- | --- |
| ● *Key* | Gets or sets the Key of the [KeyGesture](/Gui.Core/_KeyGesture.md) associated with this [KeyBinding](/Gui.Core/_KeyBinding.md) |
| ● *Modifiers* | Gets the modifier keys (one or more Alt, Ctrl, Shift) of the associated [KeyGesture](/Gui.Core/_KeyGesture.md) |
...
KeyBinding has no attached properties
