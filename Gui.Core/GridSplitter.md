...
|----------|-------------|
| IsDragging | Gets whether the Thumb control has logical focus and mouse capture and the left mouse button is pressed |
...
| Property | Description |
|----------|-------------|
| DragIncrement | Gets or sets the distance that user must drag the GridSplitter control to resize columns or rows. |
| KeyboardIncrement | Gets or sets GridSplitter's step size when moving it with arrow keys. |
| PreviewStyle | Gets or sets the Style of the GridSplitter control preview which is displayed when the ShowsPreview property is set to true. |
| ResizeBehavior | Gets or sets whichs columns or rows to resize relative to the column or row in which the GridSplitter control is definied. |
| ResizeDirection | Gets or sets whether to resize columns or rows. |
| ShowsPreview | Gets or sets whether column or row size should be updated as the GridSplitter is being dragged. |
...
| DragDelta | Occurs one or more times as the mouse changes position when a Thumb control has logical focus and mouse capture |
| DragStarted | Occurs when a Thumb control receives logical focus and mouse capture |
