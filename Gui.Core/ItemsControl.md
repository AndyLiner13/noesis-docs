...
| SetCurrentValue(dp, value) | Sets the current value of a dependency property. The current value is set on coerce field, without modifying source or animated value |
...
| ItemsControlFromItemContainer(container) | Returns the ItemsControl that owns the specified container element. Returns null if no such container exists |
