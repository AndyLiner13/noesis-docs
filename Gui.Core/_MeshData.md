Source: https://www.noesisengine.com/docs/Gui.Core._MeshData.html
...
[MeshData](_MeshData.md) allows direct submission of geometry to the GPU with no extra preprocessing. Each vertex typically includes a position and UV coordinates, and triangles are defined via index arrays. This makes the class ideal for rendering dynamic geometry efficiently, especially when per-frame updates are required.
...
| ○ *Indices* | Returns a pointer to the internal index buffer |
| ○ *UVs* | Returns a pointer to the internal texture coordinates buffer |
| ○ *Vertices* | Returns a pointer to the internal vertex buffer |
...
MeshData has no attached properties

# Methods

| Name | Description |
| --- | --- |
|  *SetNumIndices(numIndices)* | Sets number of indices |
|  *SetNumUVs(numUVs)* | Sets number of texture coordinates |
|  *SetNumVertices(numVertices)* | Sets number of vertices |
|  *Updated(flags)* | Notify about a change in the geometry mesh |
