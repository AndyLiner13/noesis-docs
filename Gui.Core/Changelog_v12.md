...
- Enhancement [UIElement](_UIElement.md) GetMeasureConstraint/GetArrangeConstraint removed from documentation.
...
- Fixed [Binding](_Binding.md) was not returning *FallbackValue* or default value if target property validation failed.
- Fixed Crash if element was captured during mouse over management.
- Fixed StaticResource failing when referencing a MergedDictionary in a [UserControl](_UserControl.md).
...
- Unity Internal *GetMeasureConstraint* and *GetArrangeConstraint* removed from [UIElement](_UIElement.md).
- Enhancement *PredictFocus* and *MoveFocus* added to [UIElement](_UIElement.md).
- Enhancement C# *IsInverted* flag exposed when creating TextureSource.
- Fixed BuildTool output problems when using pipes in Windows 10.
- Fixed Rare crash in DirectX11 when updating texture surfaces.
- Fixed Added validation of FontSize values.
- Fixed Crash when touch scrolling [ListBox](_ListBox.md) and ScrollUnit="Pixel".
...
- Enhancement C# Missing [ScrollViewer](_ScrollViewer.md) properties added.
...
- Fixed [ScrollViewer](_ScrollViewer.md) was not binding to all the scrolling properties of templated parent.
- Fixed Caret not showing with [ScrollViewer](_ScrollViewer.md) of [TextBox](_TextBox.md) template having null width.
- Fixed Issues with the position of [TextBox](_TextBox.md) cursor.
...
- Feature UI virtualization implemented and enabled by default for [ListView](../Gui.Controls/_ListView.md) and [ListBox](_ListBox.md) controls.
- Feature [ItemContainerGenerator](_ItemContainerGenerator.md) added and used by [ItemsControl](_ItemsControl.md) to generate item containers.
...
- Enhancement [Selector](_Selector.md) properties now stay in sync when being modified.
- Enhancement *ScrollIntoView* implemented in [ListBox](_ListBox.md).
- Enhancement *IsDeferredScrollingEnabled* implemented in [ScrollViewer](_ScrollViewer.md).
- Enhancement Added Scroll commands to [ScrollViewer](_ScrollViewer.md).
- Enhancement Added support for *IScrollInfo* and *CanContentScroll* in [ScrollViewer](_ScrollViewer.md).
...
- Unity Fixed [Button](_Button.md) command being incorrectly invoked when inside a manipulation.
- Fixed [ListBox](_ListBox.md) selection failing when using boxed values.
...
- Fixed C++ Reflection macros added to [ObservableCollection](_ObservableCollection.md) to allow dynamic casting.
- Fixed Support for carriage return when displaying text.
- Fixed [TextBox](_TextBox.md) caret shown outside control bounds when using a template without a [ScrollContentPresenter](_ScrollContentPresenter.md) .
- Fixed Incorrect caret position in right/center aligned [TextBox](_TextBox.md).
- Fixed [TextBox](_TextBox.md) with right aligned wrapped text not positioning caret correctly.
- Fixed *HorizontalContentAlignment* not working in [TextBox](_TextBox.md).
- Fixed [Slider](_Slider.md) input crashing.
...
- Fixed [Path](_Path.md) stretching not working correctly due to bug in bound calculations.
- Fixed Value converters ignored by [DataTrigger](_DataTrigger.md).
- Fixed [DataTrigger](_DataTrigger.md) not being evaluated correctly if condition fulfilled during initialization.
...
- Fixed [UserControl](_UserControl.md) not transferring *InputBindings* and *CommandBindings* from associated xaml.
- Fixed Can't set Resources property of [UserControl](_UserControl.md) when inside another xaml.
- Fixed Vertical [ProgressBar](_ProgressBar.md) not rendering the same as WPF.
...
- Fixed [ScrollContentPresenter](_ScrollContentPresenter.md) accessing destroyed scroll info during measure.
...
- Feature Implementation of [DataTrigger](_DataTrigger.md) and [MultiDataTrigger](_MultiDataTrigger.md). New [sample](https://github.com/Noesis/Tutorials/tree/master/Samples/DataTriggers) at GitHub.
...
- Enhancement [FrameworkElement](_FrameworkElement.md) value providers re-factorized as static classes to reduce memory usage.
...
- Unity Enhancement Support for extending [ResourceDictionary](_ResourceDictionary.md). Required to implement [DesignTimeResourceDictionary](https://www.noesisengine.com/forums/viewtopic.php?f=3&t=806).
...
- Fixed [TextBlock](_TextBlock.md) does not wrap when using a scale *LayoutTransform*.
...
- Feature Implemented DisplayMemberPath property in [ItemsControl](_ItemsControl.md) class.
- Feature Implemented SelectedValue and SelectedValuePath properties in [Selector](_Selector.md) class.
- Feature Implemented support for [HierarchicalDataTemplates](_HierarchicalDataTemplate.md).
- Feature Implemented [UniformGrid](_UniformGrid.md).
- Feature Added support for [TextBox](_TextBox.md) properties: IsSelectionActive, SelectionStart, SelectionLength and SelectedText.
...
- Enhancement Added DataContextChanged and Initialized events to [FrameworkElement](_FrameworkElement.md).
- Enhancement Added [PasswordBox](_PasswordBox.md) login sample.
...
- Enhancement Doc [Touch Tutorial](TouchTutorial.md) updated. Sample moved to [github repository](https://github.com/Noesis/Tutorials/tree/master/Content/Touch).
- Enhancement Doc Added a new entry explaining UniformGrid to [Panels Tutorial](LayoutPanelTutorial.md).
- Enhancement Doc [Data Binding Tutorial](DataBindingTutorial.md) commenting about HierarchicalDataTemplate, DisplayMemberPath and Indexers.
- Enhancement Doc Updated the documentation for Windows Store in the [Unity Tutorial](Unity3DTutorial.md). Steps simplified a bit.
- Unity Enhancement [DataTemplateSelector](_DataTemplateSelector.md) properly exposed to C#. It was only available in C++ before.
- Unity Enhancement Non-routed events for [UIElement](_UIElement.md) and [FrameworkElement](_FrameworkElement.md) exposed.
...
- Fixed [TextBlock](_TextBlock.md) inlines not working inside a template.
- Fixed [TextBlock](_TextBlock.md) alignment within data template not working properly.
- Fixed StringFormat parser not working properly in rare cases.
- Fixed Text is now updated on editable [ComboBox](_ComboBox.md) when writing in the [TextBox](_TextBox.md).
- Fixed Crash destroying elements after showing [ToolTips](_ToolTip.md).
- Fixed [Line](_Line.md) shape was not calculating correctly the stretch transform applied to the [LineGeometry](_LineGeometry.md).
- Fixed Circular references could be created when binding content to UserControl as DataContext.
- Fixed [ScrollViewer](_ScrollViewer.md) scrollbars not hiding or showing when content changed its size.
...
- Fixed [ToolTips](_ToolTip.md) were not being hidden after moving mouse outside their owner.
...
- Fixed [ComboBoxes](_ComboBox.md) opened inside a [ScrollViewer](_ScrollViewer.md) were also scrolling the ScrollViewer.
- Unity Fixed Access violation using List or ObservableCollection<T> with basic types as ItemsSource of a [Selector](_Selector.md) control.
- Unity Fixed [ListBox](_ListBox.md) failing to select items bound to a List or ObservableCollection<T> for non-reference types.
- Unity Fixed Removing item from data bound [ListBox](_ListBox.md) via command binding causing crashes.
...
- Enhancement Updated iOS and Android sections in [integration tutorial](SDKGuide.md). New [integration sample](https://github.com/Noesis/Tutorials/tree/master/Content/Integration/Android) for Android.
...
- Enhancement Improvements to documentation:
  ...
  - New: [A quick guide to XAML tutorial](XamlIntroduction.md).
  - New: [Data Binding Tutorial](DataBindingTutorial.md).
  - New: [Introducing Controls Tutorial](ControlsTutorial.md).
