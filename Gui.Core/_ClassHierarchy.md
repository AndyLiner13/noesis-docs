...
The *API* is grouped into two namespaces, *Noesis* with the core functionality of the framework and [NoesisApp](ApplicationTutorial.md) used by all our examples and tutorials. Most clients will only use the *Noesis* namespace and will take *NoesisApp* source code as a reference sample.

# Noesis Namespace

| Class | Description |
| --- | --- |
| [Adorner](_Adorner.md) | Abstract class that represents a [FrameworkElement](_FrameworkElement.md) that decorates another element. |
| [AdornerDecorator](_AdornerDecorator.md) | Provides an adorner layer for elements beneath it in the visual tree. |
| [AdornerLayer](_AdornerLayer.md) | Represents a surface for rendering adorners. An adorner layer is guaranteed to be at a higher Z-order than the elements being adorned, so adorners are always rendered on top of the adorned elements. |
| [AlternationConverter](_AlternationConverter.md) | Converts an integer to/from an object by applying the integer as an index to a list of objects. |
| [Animatable](_Animatable.md) | Abstract class that provides animation support. |
...
| [ApplicationCommands](_ApplicationCommands.md) | Provides a standard set of application related commands. |
| [ArcSegment](_ArcSegment.md) | Represents an elliptical arc between two points. |
...
| [BaseBinding](_BaseBinding.md) | Defines the common features for [Binding](_Binding.md). |
| [BaseBindingExpression](_BaseBindingExpression.md) | Represents the base class for all [Binding](_Binding.md) Expressions. |
| [BaseButton](_BaseButton.md) | Represents the base class for all button controls. |
| [BaseCommand](_BaseCommand.md) | Base class for commands. |
| [BaseDefinition](_BaseDefinition.md) | Defines the functionality required to support a shared-size group that is used by the ColumnDefinitionCollection and RowDefinitionCollection classes. |
| [BaseDictionary](_BaseDictionary.md) | Base class for dictionaries. |
| [BaseFreezableCollection](_BaseFreezableCollection.md) | Base class implementation for a [FreezableCollection](_FreezableCollection.md). |
...
| [BaseMenu](_BaseMenu.md) | Represents a control that defines choices for users to select. |
| [BaseMultiValueConverter](_BaseMultiValueConverter.md) | Base class for value converters used in MultiBindings. |
| [BaseSetter](_BaseSetter.md) | Represents the base class for value setters. |
| [BaseTextBox](_BaseTextBox.md) | An abstract base class that provides functionality for text editing controls. |
| [BaseTrigger](_BaseTrigger.md) | Represents the base class for specifying a conditional value within a [Style](_Style.md) object. |
| [BaseValueConverter](_BaseValueConverter.md) | Base class for value converters used in Bindings. |
...
| [BezierSegment](_BezierSegment.md) | Represents a cubic Bezier curve drawn between two points. |
| [Binding](_Binding.md) | Provides high-level access to the definition of a binding, which connects the properties of binding target objects and any data source. |
| [BindingExpression](_BindingExpression.md) | Contains information about a single instance of a [Binding](_Binding.md). |
| [BindingOperations](_BindingOperations.md) | Provides static methods to manipulate bindings. |
| [BitmapImage](_BitmapImage.md) | Provides a [BitmapSource](_BitmapSource.md) created from an image file located at the specifed [Uri](../Gui.Providers/_Uri.md). |
| [BitmapSource](_BitmapSource.md) | Represents a single, constant set of pixels at a certain size and resolution. |
| [BlurEffect](_BlurEffect.md) | A bitmap effect that blurs the target texture. |
| [Bold](_Bold.md) | An inline-level flow content element which causes content to render with a *bold* font weight. |
...
| [BooleanToVisibilityConverter](_BooleanToVisibilityConverter.md) | Represents the converter that converts *Boolean* values to and from *Visibility* enumeration values. |
| [Border](_Border.md) | Draws a border, background, or both around another element. |
| [BounceEase](../Gui.Animation/_BounceEase.md) | Represents an easing function that creates an animated bouncing effect. |
| [Brush](_Brush.md) | Defines objects used to paint graphical objects. Classes that derive from [Brush](_Brush.md) describe how the area is painted. |
| [BrushShader](_BrushShader.md) | Provides a custom brush by using a pixel shader. |
| [Brushes](_Brushes.md) | Declares a set of predefined [SolidColorBrush](_SolidColorBrush.md) objects. |
| [BulletDecorator](_BulletDecorator.md) | Represents a layout control that aligns a bullet and another visual object. |
| [Button](_Button.md) | Represents a push button which reacts to the *Click* event. |
| [Canvas](_Canvas.md) | Defines an area within which you can explicitly position child elements by using coordinates that are relative to the [Canvas](_Canvas.md) area. |
| [CheckBox](_CheckBox.md) | Represents a control that a user can select and clear. |
...
| [Collection](_Collection.md) | Represents a generic collection of objects that can be individually accessed by index. |
| [CollectionView](_CollectionView.md) | Represents a view for grouping, sorting, filtering, and navigating a data collection. |
| [CollectionViewSource](_CollectionViewSource.md) | Proxy of [CollectionView](_CollectionView.md) to be used from XAML files. |
...
| [Colors](_Colors.md) | Implements a set of predefined colors. |
| [ColumnDefinition](_ColumnDefinition.md) | Defines column-specific properties that apply to [Grid](_Grid.md) elements. |
| [CombinedGeometry](_CombinedGeometry.md) | Represents a 2D geometric shape defined by the combination of two [Geometry](_Geometry.md) objects. |
| [ComboBox](_ComboBox.md) | Represents a selection control with a drop-down list that can be shown or hidden by clicking the arrow on the control. |
| [ComboBoxItem](_ComboBoxItem.md) | Implements a selectable item inside a [ComboBox](_ComboBox.md). |
| [CommandBinding](_CommandBinding.md) | Binds a [RoutedCommand](_RoutedCommand.md) to the event handlers that implement the command. |
| [CommandManager](_CommandManager.md) | Provides command related events and is responsible for managing routed commands execution. |
| [ComponentCommands](_ComponentCommands.md) | Provides a standard set of component related commands. |
| [CompositeTransform](_CompositeTransform.md) | This class lets you apply multiple different transforms to an object. |
| [CompositeTransform3D](_CompositeTransform3D.md) | Represents 3-D scale, rotation, and translate transforms to be applied to an element. |
| [Condition](_Condition.md) | Represents a condition for the [MultiTrigger](_MultiTrigger.md) and the [MultiDataTrigger](_MultiDataTrigger.md), which apply changes to property values based on a set of conditions. |
| [ContentControl](_ContentControl.md) | Represents a control with a single piece of content. |
| [ContentPresenter](_ContentPresenter.md) | Displays the content of [ContentControl](_ContentControl.md). |
| [ContextMenu](_ContextMenu.md) | Represents a pop-up menu that enables a control to expose functionality that is specific to the context of the control. |
| [ContextMenuService](_ContextMenuService.md) | Provides the system implementation for displaying a [ContextMenu](_ContextMenu.md). |
| [Control](_Control.md) | Represents the base class for all user interactive elements. |
| [ControlTemplate](_ControlTemplate.md) | Specifies the visual structure and behavioral aspects of a [Control](_Control.md) that can be shared across multiple instances of the control. |
...
| [CroppedBitmap](_CroppedBitmap.md) | Provides an image source that crops another [BitmapSource](_BitmapSource.md). |
...
| [Cursor](_Cursor.md) | Represents the image used for the mouse pointer. |
| [Cursors](_Cursors.md) | Defines a set of default cursors. |
| [DashStyle](_DashStyle.md) | Represents the sequence of dashes and gaps that will be applied by a [Pen](_Pen.md). |
| [DataObject](_DataObject.md) | Defines a format-independent mechanism for transferring data. |
| [DataTemplate](_DataTemplate.md) | Describes the visual structure of a data object. |
| [DataTemplateSelector](_DataTemplateSelector.md) | Provides a way to choose a [DataTemplate](_DataTemplate.md) based on the data object and the data-bound element. |
| [DataTrigger](_DataTrigger.md) | Represents a trigger that applies property values or performs actions when the bound data meets a specified condition. |
| [Decorator](_Decorator.md) | Provides a base class for elements that apply effects onto or around a single child element, such as [Border](_Border.md) or [Viewbox](_Viewbox.md). |
...
| [DockPanel](_DockPanel.md) | Defines an area where you can arrange child elements either horizontally or vertically, relative to each other. |
...
| [DragDrop](_DragDrop.md) | Provides helper methods and fields for initiating drag-and-drop operations. |
| [DrawingContext](_DrawingContext.md) | Describes visual content using draw, push, and pop commands during [UIElement](_UIElement.md) *OnRender*. |
| [DropShadowEffect](_DropShadowEffect.md) | A bitmap effect that paints a drop shadow around the target texture. |
| [Duration](../Gui.Animation/_Duration.md) | Represents the duration of time that a [Timeline](../Gui.Animation/_Timeline.md) is active. |
| [DynamicResourceExtension](_DynamicResourceExtension.md) | Implements a markup extension that supports dynamic resource references made from XAML. |
| [DynamicTextureSource](_DynamicTextureSource.md) | Defines an [ImageSource](_ImageSource.md) implementation based on a dynamic texture. |
...
| [Effect](_Effect.md) | Provides a custom bitmap effect. |
...
| [Ellipse](_Ellipse.md) | Draws an ellipse. |
| [EllipseGeometry](_EllipseGeometry.md) | Represents the geometry of a circle or ellipse. |
| [EventTrigger](_EventTrigger.md) | Represents a trigger that applies a set of actions in response to an event. |
| [Expander](_Expander.md) | Represents a control that displays a header with a collapsible window to display content. |
...
| [FocusManager](_FocusManager.md) | Provides a set of static methods, attached properties, and events for determining and setting focus scopes and for setting the focused element within the scope. |
| [FontFamily](_FontFamily.md) | Represents a family of related fonts. |
| [FormattedText](_FormattedText.md) | Provides low-level control for drawing text. |
| [FrameworkElement](_FrameworkElement.md) | Provides GUI framework-level features for user interface elements. |
| [FrameworkPropertyMetadata](_FrameworkPropertyMetadata.md) | Reports or applies metadata for a dependency property, specifically adding framework-specific property system characteristics. |
| [FrameworkTemplate](_FrameworkTemplate.md) | Enables the instantiation of a tree of elements for a template. |
| [Freezable](../Gui.DependencySystem/_Freezable.md) | Defines an object that has a modifiable state and a read-only (frozen) state. |
| [FreezableCollection](_FreezableCollection.md) | Represents a collection of [DependencyObject](../Gui.DependencySystem/_DependencyObject.md), [Freezable](../Gui.DependencySystem/_Freezable.md), or [Animatable](_Animatable.md) objects. [FreezableCollection](_FreezableCollection.md) is itself an [Animatable](_Animatable.md) type. |
| [Geometry](_Geometry.md) | Classes that derive from this abstract base class define geometric shapes. [Geometry](_Geometry.md) objects can be used for clipping, hit-testing, and rendering 2D graphic data. |
| [GeometryGroup](_GeometryGroup.md) | Represents a composite geometry, composed of other [Geometry](_Geometry.md) objects. |
| [GradientBrush](_GradientBrush.md) | Defines an abstract class that describes a gradient, composed of gradient stops. Classes that inherit from [GradientBrush](_GradientBrush.md) describe different ways of interpreting gradient stops. |
| [GradientStop](_GradientStop.md) | Describes the location and color of a transition point in a gradient. |
| [GradientStopCollection](_GradientStopCollection.md) | Represents a collection of [GradientStop](_GradientStop.md) objects that can be individually accessed by index. |
| [Grid](_Grid.md) | Defines a flexible grid area that consists of columns and rows. |
| [GridLength](_GridLength.md) | Represents the length of elements that explicitly support *Star* unit types. Elements such as [ColumnDefinition](_ColumnDefinition.md) and [RowDefinition](_RowDefinition.md) use this type to describe width and height in order to support variable distribution of available space. |
| [GridSplitter](_GridSplitter.md) | Represents the control that redistributes space between columns or rows of a [Grid](_Grid.md) control. |
...
| [GroupBox](_GroupBox.md) | Represents a control that displays a frame around a group of controls with an optional caption. |
| [HeaderedContentControl](_HeaderedContentControl.md) | Provides the base implementation for all controls that contain single content and have a header. |
| [HeaderedItemsControl](_HeaderedItemsControl.md) | Represents a control that contains multiple items and has a header. |
| [HierarchicalDataTemplate](_HierarchicalDataTemplate.md) | Represents a [DataTemplate](_DataTemplate.md) that supports [HeaderedItemsControl](_HeaderedItemsControl.md), such as [TreeViewItem](_TreeViewItem.md) or [MenuItem](_MenuItem.md). |
| [Hyperlink](_Hyperlink.md) | An inline-level flow content element that hosts hyperlinks within flow content. |
| [ISealable](../Gui.DependencySystem/_ISealable.md) | This interface is shared across [Freezable](../Gui.DependencySystem/_Freezable.md), [Style](_Style.md) and Template. A sealed object is free-threaded. |
| [Image](_Image.md) | Represents a control that displays an image. |
| [ImageBrush](_ImageBrush.md) | Paints an area with an image. |
| [ImageSource](_ImageSource.md) | Represents an image object that has width and height. |
| [Inline](_Inline.md) | An abstract class that provides a base for all inline flow content elements. |
| [InlineUIContainer](_InlineUIContainer.md) | An inline-level flow content element which enables [UIElement](_UIElement.md) elements (i.e. a [Button](_Button.md)) to be embedded in flow content. |
| [InputBinding](_InputBinding.md) | Represents a binding between an [InputGesture](_InputGesture.md) and a command. |
| [InputGesture](_InputGesture.md) | Abstract class that describes input device gestures. |
...
| [Italic](_Italic.md) | An inline-level flow content element that causes content to render with an italic font style. |
| [ItemCollection](_ItemCollection.md) | Holds the list of items that constitute the content of an [ItemsControl](_ItemsControl.md). |
| [ItemContainerGenerator](_ItemContainerGenerator.md) | Generates the user interface on behalf of its host, such as an [ItemsControl](_ItemsControl.md). |
| [ItemsControl](_ItemsControl.md) | Represents a control that can be used to present a collection of items. |
| [ItemsPanelTemplate](_ItemsPanelTemplate.md) | Specifies the panel that the [ItemsPresenter](_ItemsPresenter.md) creates for the layout of the items of an [ItemsControl](_ItemsControl.md). |
| [ItemsPresenter](_ItemsPresenter.md) | Used within the template of an ItemControl to specify the place in the control's visual tree where the ItemsPanel defined by the [ItemsControl](_ItemsControl.md) is to be added. |
| [KeyBinding](_KeyBinding.md) | Binds a [KeyGesture](_KeyGesture.md) to a Command. |
| [KeyGesture](_KeyGesture.md) | Defines a keyboard combination that can be used to invoke a command. |
...
| [Keyboard](_Keyboard.md) | Represents the keyboard device. |
| [KeyboardNavigation](_KeyboardNavigation.md) | Provides logical and directional navigation between focusable objects. |
| [Label](_Label.md) | Represents the text label for a control. |
| [Line](_Line.md) | Draws a line. |
| [LineBreak](_LineBreak.md) | An inline flow content element that causes a line break to occur in flow content. |
| [LineGeometry](_LineGeometry.md) | Represents the geometry of a line. |
| [LineSegment](_LineSegment.md) | Creates a line between two points in a [PathFigure](_PathFigure.md). |
...
| [LinearGradientBrush](_LinearGradientBrush.md) | Paints an area with a linear gradient. A [LinearGradientBrush](_LinearGradientBrush.md) paints an area with a linear gradient. A linear gradient defines a gradient along a line. The line's end points are defined by the *StartPoint* and *EndPoint* properties of the linear gradient. A [LinearGradientBrush](_LinearGradientBrush.md) brush paints its *GradientStops* along this line. |
...
| [ListBox](_ListBox.md) | Contains a list of selectable items. |
| [ListBoxItem](_ListBoxItem.md) | Represents a selectable item in a [ListBox](_ListBox.md). |
...
| [LogicalTreeHelper](_LogicalTreeHelper.md) | Contains static methods useful for performing common tasks with nodes in a logical tree. |
| [MarkupExtension](_MarkupExtension.md) | Provides a base class for XAML markup extension implementations. |
...
| [MatrixTransform](_MatrixTransform.md) | Creates an arbitrary affine matrix transformation that is used to manipulate objects or coordinate systems in a 2D plane. |
| [MatrixTransform3D](_MatrixTransform3D.md) | Applies a 3D transformation matrix to an object. |
| [Menu](_Menu.md) | Represents a Windows menu control that enables you to hierarchically organize elements associated with commands and event handlers. |
| [MenuItem](_MenuItem.md) | Represents a selectable item inside a [Menu](_Menu.md) or [ContextMenu](_ContextMenu.md). |
| [Mesh](_Mesh.md) | Renders a mesh geometry defined by vertices and indexed triangles. |
| [MeshData](_MeshData.md) | Represents a low-level geometric mesh composed of vertices and indexed triangles. |
| [Mouse](_Mouse.md) | Represents the mouse device. |
| [MouseBinding](_MouseBinding.md) | Binds a [MouseGesture](_MouseGesture.md) to a Command. |
| [MouseGesture](_MouseGesture.md) | Defines a mouse input gesture that can be used to invoke a command. |
| [MultiBinding](_MultiBinding.md) | Describes a collection of [Binding](_Binding.md) objects attached to a single binding target property. |
| [MultiBindingExpression](_MultiBindingExpression.md) | Contains instance information about a single instance of a [MultiBinding](_MultiBinding.md). |
| [MultiDataTrigger](_MultiDataTrigger.md) | Represents a trigger that applies property values or performs actions when the bound data meets a set of conditions. |
| [MultiTrigger](_MultiTrigger.md) | Represents a trigger that applies property values or performs actions when a set of conditions are satisfied. |
| [NameScope](_NameScope.md) | Store relationships between the XAML defined names of objects and their instances. |
| [NullExtension](_NullExtension.md) | Implements a XAML markup extension in order to return a null object. |
...
| [ObservableCollection](_ObservableCollection.md) | Represents a dynamic data collection that provides notifications when items get added or removed. |
| [Page](_Page.md) | Encapsulates a page of content. |
| [Panel](_Panel.md) | Provides a base class for all [Panel](_Panel.md) elements. Use [Panel](_Panel.md) elements to position and arrange child objects. |
...
| [PasswordBox](_PasswordBox.md) | Represents a control designed for entering and handling passwords. |
| [Path](_Path.md) | Draws a series of connected lines and curves. |
| [PathFigure](_PathFigure.md) | Represents a subsection of a geometry, a single connected series of two-dimensional segments. |
| [PathGeometry](_PathGeometry.md) | Represents a complex shape that may be composed of arcs, curves, ellipses, lines and rectangles. |
| [PathSegment](_PathSegment.md) | Represents a complex shape that may be composed of arcs, curves, ellipses, lines and rectangles. |
| [PauseStoryboard](../Gui.Animation/_PauseStoryboard.md) | A trigger action that pauses a [Storyboard](../Gui.Animation/_Storyboard.md). |
| [Pen](_Pen.md) | Describes how a shape is outlined. |
...
| [PolyBezierSegment](_PolyBezierSegment.md) | Represents one or more cubic Bezier curves. |
| [PolyLineSegment](_PolyLineSegment.md) | Represents a set of line segments defined by a PointCollection with each Point specifying the end point of a line segment. |
| [PolyQuadraticBezierSegment](_PolyQuadraticBezierSegment.md) | Represents one or more quadratic Bezier curves. |
| [Popup](_Popup.md) | Represents a pop-up window that has content. |
...
| [ProgressBar](_ProgressBar.md) | Indicates the progress of an operation. |
...
| [PropertyPath](_PropertyPath.md) | Implements a data structure for describing a property as a path below another property, or below an owning type. Property paths are used in data binding to objects, and in storyboards and timelines for animations. |
| [QuadraticBezierSegment](_QuadraticBezierSegment.md) | Represents a quadratic Bezier curve drawn between two points. |
...
| [RadialGradientBrush](_RadialGradientBrush.md) | Paints an area with a radial gradient. |
| [RadioButton](_RadioButton.md) | Represents a button that can be selected, but not cleared, by a user. |
| [RangeBase](_RangeBase.md) | Represents an element that has a value within a specific range. |
...
| [Rectangle](_Rectangle.md) | Draws a rectangle. |
| [RectangleGeometry](_RectangleGeometry.md) | Describes a two-dimensional rectangle geometry. |
| [RelativeSource](_RelativeSource.md) | Implements a markup extension that describes the location of the binding source relative to the position of the binding target. |
| [RenderOptions](_RenderOptions.md) | Provides options for controlling the rendering behavior of objects. |
...
| [RepeatButton](_RepeatButton.md) | Represents a control that raises its *Click* event repeatedly from the time it is pressed until it is released. |
| [ResourceDictionary](_ResourceDictionary.md) | Provides a hash table implementation that contains resources used by the UI. |
...
| [RotateTransform](_RotateTransform.md) | Rotates an object clockwise about a specified point in 2D coordinate system. |
| [RoutedCommand](_RoutedCommand.md) | A command that is routed through the element tree. |
| [RoutedEvent](_RoutedEvent.md) | An event instance that propagates through a tree of related elements. There are three types of routing: bubbling, tunneling and direct. With bubbling, the event instance moves from the source of the event up to the top of the tree. With tunneling, the event instance starts at the top of the tree and moves down to the source of the event. With direct routing, the event instance behaves like a standard event. |
| [RoutedUICommand](_RoutedUICommand.md) | A command that is routed through the element tree and contains a text property. |
| [RowDefinition](_RowDefinition.md) | Defines row-specific properties that apply to [Grid](_Grid.md) elements. |
| [Run](_Run.md) | An inline-level flow content element intended to contain a run of formatted or unformatted text. |
| [ScaleTransform](_ScaleTransform.md) | Scales an object in the 2D coordinate system. |
| [ScrollBar](_ScrollBar.md) | Represents a control that provides a scroll bar that has a sliding [Thumb](_Thumb.md) whose position corresponds to a value. |
| [ScrollContentPresenter](_ScrollContentPresenter.md) | Displays the content of a [ScrollViewer](_ScrollViewer.md) control. |
| [ScrollViewer](_ScrollViewer.md) | Represents a scrollable area that can contain other visible elements. |
| [Selector](_Selector.md) | Represents a control that allows a user to select items from among its child elements. |
| [Separator](_Separator.md) | [Control](_Control.md) that is used to separate items in items controls. |
| [Setter](_Setter.md) | Represents a setter that applies a property value. |
| [ShaderEffect](_ShaderEffect.md) | Provides a custom bitmap effect by using a pixel shader. |
| [Shape](_Shape.md) | Provides a base class for shape elements. |
...
| [SkewTransform](_SkewTransform.md) | Represents a 2D skew. It is useful for creating the illusion of 3-dimensional depth in a 2D object. |
| [Slider](_Slider.md) | Represents a control that lets the user select from a range of values by moving a [Thumb](_Thumb.md) control along a [Track](_Track.md). |
| [SolidColorBrush](_SolidColorBrush.md) | Paints an area with a solid color. |
| [Span](_Span.md) | Groups other [Inline](_Inline.md) content elements. |
...
| [StackPanel](_StackPanel.md) | Arranges child elements into a single horizontal or vertical line. |
| [StaticExtension](_StaticExtension.md) | Implements a markup extension that returns static field and property references. |
| [StaticResourceExtension](_StaticResourceExtension.md) | Implements a markup extension that supports static resource references made from XAML. |
| [StatusBar](_StatusBar.md) | Represents a control that displays items and information in a horizontal bar (typically at the bottom) in an application window. |
| [StatusBarItem](_StatusBarItem.md) | Represents an item of a [StatusBar](_StatusBar.md) control. |
...
| [StreamGeometry](_StreamGeometry.md) | Defines a geometric shape, described using a [StreamGeometryContext](_StreamGeometryContext.md) or a SVG command string as described in the [W3C](http://www.w3.org/TR/SVG11/paths.html) specs. |
| [StreamGeometryContext](_StreamGeometryContext.md) | Describes a geometry using drawing commands. This class is used with the [StreamGeometry](_StreamGeometry.md) class to create a lightweight geometry that does not support data binding, animation, or modification. |
...
| [Style](_Style.md) | Enables the sharing of properties, resources, and event handlers between instances of a type. |
| [TabControl](_TabControl.md) | Represents a control that contains multiple items that share the same space on the screen. |
| [TabItem](_TabItem.md) | Represents a selectable item inside a [TabControl](_TabControl.md). |
| [TabPanel](_TabPanel.md) | Handles the layout of the [TabItem](_TabItem.md) objects on a [TabControl](_TabControl.md). |
| [TemplateBindingExpression](_TemplateBindingExpression.md) | Describes a run-time instance of a [TemplateBindingExtension](_TemplateBindingExtension.md). |
| [TemplateBindingExtension](_TemplateBindingExtension.md) | Implements a markup extension that supports the binding between the value of a property in a template and the value of some other exposed property on the templated control. |
| [TextBlock](_TextBlock.md) | Provides a lightweight control for displaying small amounts of text content. |
| [TextBox](_TextBox.md) | Represents a control that can be used to display or edit unformatted text. |
| [TextElement](_TextElement.md) | Defines text attached properties. |
| [Texture](../Render.RenderDevice/_Texture.md) | Base class for 2D textures |
| [TextureSource](_TextureSource.md) | Defines a [BitmapSource](_BitmapSource.md) constructed from a [Texture](../Render.RenderDevice/_Texture.md). |
...
| [Thumb](_Thumb.md) | Represents a control that can be dragged by the user. |
| [TickBar](_TickBar.md) | Represents a control that draws a set of tick marks for a [Slider](_Slider.md) control. |
| [TileBrush](_TileBrush.md) | Describes a way to paint a region by using one or more tiles. |
...
| [ToggleButton](_ToggleButton.md) | Base class for button controls that can switch states. |
| [ToolBar](_ToolBar.md) | Provides a container for a group of commands or controls. |
| [ToolBarOverflowPanel](_ToolBarOverflowPanel.md) | Used to arrange overflow [ToolBar](_ToolBar.md) items. |
| [ToolBarPanel](_ToolBarPanel.md) | Arranges [ToolBar](_ToolBar.md) items inside a [ToolBar](_ToolBar.md). |
| [ToolBarTray](_ToolBarTray.md) | Represents the container that handles the layout of a [ToolBar](_ToolBar.md). |
| [ToolTip](_ToolTip.md) | Represents a control that creates a pop-up window that displays information for an element in the interface. |
| [ToolTipService](_ToolTipService.md) | Represents a service that provides properties and events to control the display and behavior of tooltips. |
| [Track](_Track.md) | Represents a control primitive that handles the positioning and sizing of a [Thumb](_Thumb.md) control and two [RepeatButton](_RepeatButton.md) controls that are used to set a Value. |
| [Transform](_Transform.md) | Defines functionality that enables transformations in a 2D plane. |
| [Transform3D](_Transform3D.md) | Provides a base class for all three-dimensional transformations, including translation, rotation, and scale transformations. |
| [TransformGroup](_TransformGroup.md) | Represents a composite [Transform](_Transform.md) composed of other [Transform](_Transform.md) objects. |
| [TranslateTransform](_TranslateTransform.md) | Translates an object in the 2D coordinate system. |
| [TreeView](_TreeView.md) | Represents a control that displays hierarchical data in a tree structure that has items that can expand and collapse. |
| [TreeViewItem](_TreeViewItem.md) | Implements a selectable item in a [TreeView](_TreeView.md) control. |
| [Trigger](_Trigger.md) | Represents a trigger that applies property values or performs actions conditionally. |
| [TriggerAction](_TriggerAction.md) | Describes an action to perform for a trigger. |
| [TypeExtension](_TypeExtension.md) | Implements a markup extension that returns a *Type* based on a string input. |
| [Typography](_Typography.md) | Provides access to a rich set of OpenType typography properties. |
| [UIElement](_UIElement.md) | Provides a starting point for element layout characteristics, and also exposes virtual methods that derived classes can override, which can influence the layout rendering behavior of the element and its child elements. |
| [UIElementCollection](_UIElementCollection.md) | Represents an ordered collection of [UIElement](_UIElement.md) child elements. |
| [UIPropertyMetadata](_UIPropertyMetadata.md) | Provides property metadata for non-framework properties that do have rendering/user interface impact at the core level. |
| [Underline](_Underline.md) | An inline-level flow content element which causes content to render with an underlined text decoration. |
| [UniformGrid](_UniformGrid.md) | Provides a way to arrange content in a grid where all the cells in the grid have the same size. |
| [Uri](../Gui.Providers/_Uri.md) | Provides a representation of a uniform resource identifier and easy access to parts of the URI. |
| [UserControl](_UserControl.md) | Provides a simple way to create a composition of controls. |
| [ValueTargetProvider](_ValueTargetProvider.md) | Provides useful information during [MarkupExtension](_MarkupExtension.md) *ProvideValue* calls. |
| [Viewbox](_Viewbox.md) | Defines a content decorator that can stretch and scale a single child to fill the available space. |
| [VirtualizationCacheLength](_VirtualizationCacheLength.md) | Represents the measurements for the *VirtualizingPanel.CacheLength* attached property. |
| [VirtualizingPanel](_VirtualizingPanel.md) | Base class that provides access to information that is useful for implementing virtualization. |
| [VirtualizingStackPanel](_VirtualizingStackPanel.md) | Arranges and virtualizes content on a single line that is oriented either horizontally or vertically. |
| [VirtualizingWrapPanel](_VirtualizingWrapPanel.md) | A high-performance [WrapPanel](_WrapPanel.md) that supports UI virtualization, designed for use in both horizontal and vertical layouts. |
| [Visual](_Visual.md) | Provides rendering support, which includes hit testing, coordinate transformation, and bounding box calculations. |
| [VisualBrush](_VisualBrush.md) | Paints an area with a [Visual](_Visual.md). |
| [VisualCollection](_VisualCollection.md) | Represents an ordered collection of [Visual](_Visual.md) objects. |
...
| [VisualTreeHelper](_VisualTreeHelper.md) | Provides utility methods that perform common tasks involving nodes in a visual tree. |
| [WrapPanel](_WrapPanel.md) | Positions child elements in sequential position from left to right, breaking content to the next line at the edge of the containing box. |
...
| [GamepadTrigger](../App.Interactivity/_GamepadTrigger.md) | A [Trigger](_Trigger.md) that is triggered by a specified gamepad button. |
| [GoToStateAction](../App.Interactivity/_GoToStateAction.md) | An action that will transition a [FrameworkElement](_FrameworkElement.md) to a specified [VisualState](../Gui.Animation/_VisualState.md) when invoked. |
...
| [KeyTrigger](../App.Interactivity/_KeyTrigger.md) | A [Trigger](_Trigger.md) that is triggered by a keyboard event. If *Key* and *Modifiers* are detected, it fires. |
...
| [LineDecorationBehavior](../App.Interactivity/_LineDecorationBehavior.md) | Renders a line decoration on the associated [TextBlock](_TextBlock.md). |
| [LoadContentAction](../App.Interactivity/_LoadContentAction.md) | An action that will set the Content of the target [ContentControl](_ContentControl.md) by loading a xaml. |
| [LocExtension](../App.ApplicationLauncher/_LocExtension.md) | Implements a markup extension that supports references to a localization [ResourceDictionary](_ResourceDictionary.md). |
...
| [MonochromeBrush](../App.Shaders/_MonochromeBrush.md) | [BrushShader](_BrushShader.md) that turns an image into a monochrome color |
...
| [PinchEffect](../App.Shaders/_PinchEffect.md) | [Effect](_Effect.md) to warp or pinch a specific area in a visual. |
| [PixelateEffect](../App.Shaders/_PixelateEffect.md) | [ShaderEffect](_ShaderEffect.md) that pixelates a visual. |
| [PlasmaBrush](../App.Shaders/_PlasmaBrush.md) | [BrushShader](_BrushShader.md) for old school plasma effect. |
...
| [RichText](../App.ApplicationLauncher/_RichText.md) | Adds a *Text* attached property for [TextBlock](_TextBlock.md) which formats [BBCode](https://www.bbcode.org/reference.php) into Inlines. |
...
| [StyleInteraction](../App.Interactivity/_StyleInteraction.md) | Allows setting a collection of *Interactivity* behaviors and triggers in a [Style](_Style.md), so they are applied to all instances of the styled control. |
...
| [WavesBrush](../App.Shaders/_WavesBrush.md) | [BrushShader](_BrushShader.md) that paints a PSP wavy background effect. |
