...
## namespace [Noesis](_ClassHierarchy.md#noesis-namespace) | [MSDN](http://msdn.microsoft.com/en-us/library/system.windows.controls.button.aspx)
...
| Name | Description |
| --- | --- |
| ● *IsCancel* | Gets or sets a value that indicates whether a [Button](_Button.md) is a Cancel button. A user can activate the Cancel button by pressing the *ESC* key |
| ● *IsDefault* | Gets or sets a value that indicates whether a [Button](_Button.md) is the default button. A user invokes the default button by pressing the *ENTER* key |
| ● *IsDefaulted* | Gets a value that indicates whether a [Button](_Button.md) is the button that is activated when a user presses *ENTER* key |
