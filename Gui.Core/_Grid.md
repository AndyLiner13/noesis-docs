...
## namespace [Noesis](_ClassHierarchy.md#noesis-namespace) | [MSDN](http://msdn.microsoft.com/en-us/library/system.windows.controls.grid.aspx)
...
You can precisely position child elements of a [Grid](_Grid.md) by using a combination of the Margin property and alignment properties.

Child elements of a [Grid](_Grid.md) are drawn in the order in which they appear in markup or code. As a consequence, layered order (also known as z-order) can be achieved when elements share the same coordinates.

[Grid](_Grid.md) uses the [GridLength](_GridLength.md) object to define the sizing characteristics of a [RowDefinition](_RowDefinition.md) or [ColumnDefinition](_ColumnDefinition.md). For a definition of each unit type, see GridUnitType.

If a child element is added to a column within a [Grid](_Grid.md), and the column has its Width property set to Auto, the child will be measured without restrictions. This behavior can prevent horizontal scroll bars from displaying if a [ScrollViewer](_ScrollViewer.md) is being used, as the child element is measured as unbounded. For purposes of display, the child is clipped rather than scrolled.
...
| Name | Description |
| --- | --- |
| ○ *ColumnDefinitions* | Gets a ColumnDefinitionCollection defined on this instance of [Grid](_Grid.md) |
| ○ *RowDefinitions* | Gets a RowDefinitionCollection defined on this instance of [Grid](_Grid.md) |
...
| Name | Description |
| --- | --- |
| ● *Column* | Gets or sets a value that indicates which column child within a [Grid](_Grid.md) should appear in |
| ● *ColumnSpan* | Gets or sets a value that indicates the number of columns that child spans within a [Grid](_Grid.md) |
| ● *IsSharedSizeScope* | Gets or sets a value that indicates that multiple [Grid](_Grid.md) elements are sharing size information |
| ● *Row* | Gets or sets a value that indicates which row child within a [Grid](_Grid.md) should appear in |
| ● *RowSpan* | Gets or sets a value that indicates the number of rows that child spans within a [Grid](_Grid.md) |
