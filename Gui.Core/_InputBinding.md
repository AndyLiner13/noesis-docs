...
## namespace [Noesis](/Gui.Core/_ClassHierarchy.md#noesis-namespace) | [MSDN](http://msdn.microsoft.com/en-us/library/system.windows.input.inputbinding.aspx)

Represents a binding between an [InputGesture](/Gui.Core/_InputGesture.md) and a command.

You can specify that user input invokes a command by creating a [InputBinding](/Gui.Core/_InputBinding.md). When the user performs the specified input, the ICommand that is set to the *Command* property is executed.

You can specify that the [InputBinding](/Gui.Core/_InputBinding.md) invokes a command that is defined on an object by creating a binding on the *Command*, *CommandParameter*, and *CommandTarget* properties. This enables you to define a custom command and associate it with user input.
...
• [KeyBinding](/Gui.Core/_KeyBinding.md)

• [MouseBinding](/Gui.Core/_MouseBinding.md)
