...
Derive from the [BrushShader](_BrushShader.md) class to implement a custom brush based on a pixel shader.

In comparison with [ShaderEffect](_ShaderEffect.md), a brush shader is more efficient because it doesn't need an extra render pass to an offscreen.
