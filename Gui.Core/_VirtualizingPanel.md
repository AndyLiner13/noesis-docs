Source: https://www.noesisengine.com/docs/Gui.Core._VirtualizingPanel.html

# VirtualizingPanel Class

## namespace [Noesis](/Gui.Core/_ClassHierarchy.md#noesis-namespace) | [MSDN](http://msdn.microsoft.com/en-us/library/system.windows.controls.virtualizingpanel.aspx)

Base class that provides access to information that is useful for implementing virtualization.

In this context, "virtualize" refers to a technique by which a subset of user interface (UI) elements are generated from a larger number of data items based on which items are visible on the screen. It is intensive, both in terms of memory and processor, to generate a large number of UI elements when only a few may be on the screen at a given time. A panel that inherits from [VirtualizingPanel](/Gui.Core/_VirtualizingPanel.md), such as the [VirtualizingStackPanel](/Gui.Core/_VirtualizingStackPanel.md), calculates visible items and works with the [ItemContainerGenerator](/Gui.Core/_ItemContainerGenerator.md) from an [ItemsControl](/Gui.Core/_ItemsControl.md) (such as [ListBox](/Gui.Core/_ListBox.md) or [ListView](/Gui.Controls/_ListView.md)) to only create UI elements for visible items.
...
• [VirtualizingStackPanel](/Gui.Core/_VirtualizingStackPanel.md)

• [VirtualizingWrapPanel](/Gui.Core/_VirtualizingWrapPanel.md)
