...
## namespace [Noesis](_ClassHierarchy.md#noesis-namespace) | [MSDN](http://msdn.microsoft.com/en-us/library/system.windows.controls.toolbartray.aspx)

Represents the container that handles the layout of a [ToolBar](_ToolBar.md).

A [ToolBarTray](_ToolBarTray.md) is responsible for handling placement, sizing, drag-and-drop operations, and rearranging of [ToolBar](_ToolBar.md) controls. [ToolBarTray](_ToolBarTray.md) controls are also responsible for managing the rows (or "bands") in which [ToolBar](_ToolBar.md) controls appear. A [ToolBarTray](_ToolBarTray.md) may be horizontal or vertical and, like a [Menu](_Menu.md), is often docked at the top of an application window.
...
| Name | Description |
| --- | --- |
| ● *Background* | Gets or sets a brush to use for the background color of the [ToolBarTray](_ToolBarTray.md) |
| ● *Orientation* | Specifies the orientation of a [ToolBarTray](_ToolBarTray.md) |
...
| Name | Description |
| --- | --- |
| ● *IsLocked* | Gets or sets a value that indicates whether a [ToolBar](_ToolBar.md) can be moved inside a [ToolBarTray](_ToolBarTray.md) |
