...
- Enhancement Improved [Setter](/Gui.Core/_Setter.md), [Trigger](/Gui.Core/_Trigger.md) and [Condition](/Gui.Core/_Condition.md) error messages when value type is incorrect.
- Fixed C# Crash during shutdown deleting C# proxies still having a C++ reference.

# Version 2.0.2f1

- Feature Support for Xbox One in Unity.
- Feature Unity Support for game pads ([more info](/Gui.Core/Unity3DTutorial.md#game-pad)).
- Feature Implemented [Underline](/Gui.Core/_Underline.md) and [Hyperlink](/Gui.Core/_Hyperlink.md) inlines.
...
- Enhancement Added cursor property to [TextBox](/Gui.Core/_TextBox.md), [PasswordBox](/Gui.Core/_PasswordBox.md) and [Hyperlink](/Gui.Core/_Hyperlink.md) in our styles.
- Documentation Improved [performance optimization tutorial](/Gui.Core/Optimizing.md).
- Documentation [UserControl tutorial](/Gui.Core/UserControlTutorial.md) adapted to NoesisGUI 2.0.
- Documentation [Unity3D tutorial](/Gui.Core/Unity3DTutorial.md) improved with mentions to game pads and texture import settings.
...
- Fixed Crash when pressing space in a focused [ComboBox](/Gui.Core/_ComboBox.md).
...
- Feature *Cursor* property implemented in [FrameworkElement](/Gui.Core/_FrameworkElement.md). *SetCursorCallback* added to integration API.
- Enhancement Support for RGBA8 and RGB besides BGRA8 in [BitmapSource](/Gui.Core/_BitmapSource.md).
- Enhancement Improved error messages in XAML parser. Added line and column information whenever possible.
- Enhancement Improved how [BindingExpression](/Gui.Core/_BindingExpression.md) unregister sources to avoid unexpected errors.
...
- Fixed [ContextMenu](/Gui.Core/_ContextMenu.md) no longer allowing logical or visual parent.
- Fixed Some *ItemsControl* default templates were not using [ItemsPresenter](/Gui.Core/_ItemsPresenter.md).
- Fixed Crash closing application while [ContextMenu](/Gui.Core/_ContextMenu.md) open.
- Fixed Registered named elements not found when [UserControl](/Gui.Core/_UserControl.md) is root of a template.
...
- Fixed [UserControl](/Gui.Core/_UserControl.md) inside a template was ignoring any *Resource* or *Trigger* defined in the associated XAML.
- Fixed *Caret* selection and positioning was wrong in multiline [TextBox](/Gui.Core/_TextBox.md).
...
- Fixed Crash in [TextBlock](/Gui.Core/_TextBlock.md) due to uninitialized software keyboard callbacks.
...
- Fixed Unity *EnablePostProcess* property deprecated. Must use multiple cameras now. Unity [tutorial](/Gui.Core/Unity3DTutorial.md) updated.
...
- Doc Unity [tutorial](/Gui.Core/Unity3DTutorial.md) documentation updated.
...
- Enhancement Added support for *Paste* and *SelectAll* commands in [PasswordBox](/Gui.Core/_PasswordBox.md).
- Enhancement Support for showing [ToolTip](/Gui.Core/_ToolTip.md) on disabled elements.
...
- Fixed [TextBox](/Gui.Core/_TextBox.md) inside [ListBox](/Gui.Core/_ListBox.md) was losing focus when space key was pressed.

# Version 2.0.0b4

- Feature Apple Metal renderer. Integration sample added to SDK.
- Feature Implemented support for events in code-behind. New tutorial on [events](/Gui.Core/EventsTutorial.md).
...
- Enhancement Improvements to [TreeView](/Gui.Core/_TreeView.md) creation times.
...
- Doc Added note to [Unity tutorial](/Gui.Core/Unity3DTutorial.md#building-on-android) about problems using Android OBBs.
- Fixed Due to layout issues [ListBox](/Gui.Core/_ListBox.md) shows empty even though it is populated with data.
- Fixed First element of the [ListBox](/Gui.Core/_ListBox.md) was moved 1 pixel down when scrolling.
- Fixed [TextBlock](/Gui.Core/_TextBlock.md) not updating layout after a new measure, rendering nothing.
- Fixed Problems disposing C# proxy instances.
- Fixed [ComboBox](/Gui.Core/_ComboBox.md) selecting incorrect item after scrolling a long list with mouse wheel.
- Fixed Duplicate name registration when having the same [UserControl](/Gui.Core/_UserControl.md) several times in a template.
- Fixed Crash if caret arranged before [TextBox](/Gui.Core/_TextBox.md).
...
- Fixed Crash when focusing a [TextBox](/Gui.Core/_TextBox.md) before doing its layout.
- Fixed Crash in [TreeView](/Gui.Core/_TreeView.md) when clearing ItemsSource collection.
- Fixed Invalid generated geometry when stroking an empty [Rectangle](/Gui.Core/_Rectangle.md).

# Version 2.0.0b3

- Feature D3D11 Support for activating linear space rendering (sRGB) in *D3D11RenderDevice* constructor.
- Feature Support for creating [ImageSource](/Gui.Core/_ImageSource.md) from memory buffer using BitmapSource::Create.
- Feature Support for creating [TextureSource](/Gui.Core/_TextureSource.md) from native (GL, D3D) handles.
- Feature [BindingExpression](/Gui.Core/_BindingExpression.md) exposed to public API.
...
- Enhancement Improved performance when rendering [VisualBrush](/Gui.Core/_VisualBrush.md).
- Enhancement Added support for high-resolution mouse wheels.
- Enhancement [Popup](/Gui.Core/_Popup.md) *Opened* and *Closed* events exposed to C#.
- Fixed [Path](/Gui.Core/_Path.md) hit testing failing sometimes with multiple figures.
- Fixed Glyph kerning was sometimes incorrect at the beginning of a line.
- Fixed [ToolBar](/Gui.Core/_ToolBar.md) not measuring correctly children.
- Fixed [ContextMenu](/Gui.Core/_ContextMenu.md) opening and closing events now matches WPF order.

# Version 2.0.0b2

- Enhancement Visual Studio 2015 officially supported.
- Xbox One C++ New platform supported.
- Doc New [resource providers tutorial](/Gui.Core/ProvidersTutorial.md).
- Doc Tutorial for the new [rendering architecture](/Gui.Core/RenderingTutorial.md).
...
- Fixed Pressing *Up* arrow in empty [TextBox](/Gui.Core/_TextBox.md) unfocus the control.
- C++ Deprecated *NsFunc* no longer supported in reflection macros. Events must be manually registered.

# Version 2.0.0b1

- Feature New multithreading rendering architecture. Class *IRenderer* split into *IView* (for the main thread) and *IRenderer* (for the renderer thread, if any). More information at the [rendering tutorial](/Gui.Core/RenderingTutorial.md).
...
- Feature New resource providers architecture:
  ...
  - [Custom loaders](/Gui.Core/ProvidersTutorial.md) must be passed to *Noesis::GUI::SetResourceProvider()*. We support local files by default (similar to 1.2).
