Source: https://www.noesisengine.com/docs/Gui.Core._AdornerLayer.html

# AdornerLayer Class

## namespace [Noesis](/Gui.Core/_ClassHierarchy.md#noesis-namespace) | [MSDN](https://docs.microsoft.com/en-us/dotnet/api/system.windows.documents.adornerlayer)

Represents a surface for rendering adorners. An adorner layer is guaranteed to be at a higher Z-order than the elements being adorned, so adorners are always rendered on top of the adorned elements.
...
AdornerLayer has no attached properties

# Methods

| Name | Description |
| --- | --- |
|  *Add(adorner)* | Adds an adorner to the adorner layer |
|  *GetAdornerLayer(visual)* | Returns the first adorner layer in the visual tree above a specified [Visual](/Gui.Core/_Visual.md) |
|  *Remove(adorner)* | Removes the specified [Adorner](/Gui.Core/_Adorner.md) from the adorner layer |
|  *Update()* | Updates the layout and redraws all of the adorners in the adorner layer |
|  *Update(element)* | Updates the layout and redraws all of the adorners in the adorner layer that are bound to the specified element |
