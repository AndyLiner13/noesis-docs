...
In both cases, a pixel shader compatible with the active *RenderDevice* is required. Our *Application Framework* provides a [ShaderCompiler](ApplicationTutorial.md#shader-compiler) tool that is used in C++ and Unity. For Unreal, the native [Shader Graph](UnrealTutorial.md#materials) editor must be used.

# Shader Effects

NoesisGUI has two special visual effects built in that can be applied to any [Visual](_Visual.md). These effects are [DropShadowEffect](_DropShadowEffect.md) and [BlurEffect](_BlurEffect.md), which both derive from the abstract [Effect](_Effect.md) class.
...
The exciting part is not necessarily the two built-in ones but a third [Effect](_Effect.md) subclass called [ShaderEffect](_ShaderEffect.md) that enables you to easily inject your own custom effects. By deriving from the abstract [ShaderEffect](_ShaderEffect.md) class, you can apply any pixel shader to any [UIElement](_UIElement.md) of the tree.

There are many steps necessary to create a working shader effect class. To ease the development of custom shaders, the [Application Framework](ApplicationTutorial.md) provides the following effects, we recommend inspecting the source code of these effects while reading this tutorial:
...
![ShadersTutorialImg1.jpg](https://www.noesisengine.com/docs/ShadersTutorialImg1.jpg)

## Creating Custom Effects

[ShaderEffect](_ShaderEffect.md) is the abstract class that serves as a base for your custom effect class. It is a dependency object, so you can populate it with dependency properties to define the parameters of your effect. As described above, it works in conjunction with a pixel shader.
...
An effect is normally applied to an element's actual render size. Therefore an effect for a 200×200 Image will modify pixels in a 200×200 region. Certain effects, like the drop shadow, need additional space outside the normal render area. Use the [ShaderEffect](_ShaderEffect.md) *SetPadding* function to increase the size required in the pixel shader.

# Shader Brushes

Apart from *Effects*, NoesisGUI provides functionality that allows [ImageBrush](_ImageBrush.md) to be extended by using pixel shaders. By deriving from the abstract [BrushShader](_BrushShader.md) class, you can generate procedural pattern brushes driven by a pixel shader.

In comparison with [ShaderEffect](_ShaderEffect.md), a brush shader is more efficient because it doesn't need an extra render pass to an offscreen. But on the other hand, it cannot be applied to a tree of elements.

To apply a shader brush to an [ImageBrush](_ImageBrush.md), you set its *Brush.Shader* property. Note that this is an [Extension](ExtensionsTutorial.md) to WPF provided by NoesisGUI.
...
The [Application Framework](ApplicationTutorial.md) provides the following shader brushes as examples:
...
[BrushShader](_BrushShader.md) is the abstract class that serves as a base for your custom brush. It is a dependency object, so you can populate it with dependency properties to configure your shader. Same as in [ShaderEffect](_ShaderEffect.md), this class works in conjunction with a pixel shader.
