Source: https://www.noesisengine.com/docs/App.ApplicationLauncher._Window.html

# Window Class

## namespace [NoesisApp](/Gui.Core/_ClassHierarchy.md#noesisapp-namespace) | [MSDN](http://msdn.microsoft.com/en-us/library/system.windows.window.aspx)
...
| ○ *Display* | Gets window display |
| ● *Fullscreen* | Gets or sets window fullscreen flag |
...
| ● *Title* | Gets or sets window title |
| ● *Top* | Gets or sets the position of the window's top edge, in relation to the desktop |
| ● *Topmost* | Gets or sets a value that indicates whether window appears in the topmost z-order |
| ● *WindowStartupLocation* | Gets or sets the position of the window when first shown |
| ● *WindowState* | Gets or sets a value that indicates whether window is restored, minimized, or maximized |
| ● *WindowStyle* | Gets or sets window border style |
...
Window has no attached properties
...
|  *Init(display, context, samples, ppaa, lcd, emulateTouch)* | Initializes the window with the given display and render context |
|  *LocationChanged()* | Occurs when the window's location changes |
|  *RecreateView(context, emulateTouch)* | Creates a new view in the given render context |
|  *Render(time)* | Renders frame at the given time |
|  *ResetView()* | Destroys the active view |
