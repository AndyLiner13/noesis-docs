Source: https://www.noesisengine.com/docs/App.RiveBase._RiveControl.html

# RiveControl Class

## namespace [NoesisApp](/Gui.Core/_ClassHierarchy.md#noesisapp-namespace)

Renders a [Rive](https://rive.app/) scene.

The scene can contain inputs to control the state machine and animations. This control exposes a list of [RiveInput](/App.RiveBase/_RiveInput.md) to set the values of those inputs in XAML.
...
The control also exposes a list of [RiveRun](/App.RiveBase/_RiveRun.md) to interact with the text runs defined in the Rive scene. The text can be bound to set or get run's current value.
...
The UI can fire triggers defined in the Rive scene using the interactivity [RiveTriggerAction](/App.Rive/_RiveTriggerAction.md).
...
| Name | Description |
| --- | --- |
| ○ *Inputs* | Gets the collection of inputs that will control the state machine |
| ○ *Runs* | Gets the collection of text runs used to interact with the scene runs |
| ● *Source* | Gets or sets the Rive scene source |
| ● *State* | Gets the name of the current state in the state machine |
| ● *Stretch* | Gets or sets a Stretch value that describes how the Rive scene fills its allocated space |
...
RiveControl has no attached properties

# Methods

| Name | Description |
| --- | --- |
|  *FireInputTrigger(name)* | Fires a trigger input in the Rive scene |
|  *GetSourceInput(index, type)* | Gets information about the specified input |
|  *GetSourceInputCount()* | Gets the number of available inputs in the associated Rive scene |
|  *GetSourceRunCount()* | Gets the number of available text runs in the associated Rive scene |
|  *GetSourceRunName(index)* | Gets the name of the specified text run element |
|  *SetInputValue(path, name, value)* | Sets the value of an input in the Rive scene. If a path is provided it indicates the name of the nested Artboard where the input should be found |
|  *SetRunText(name, text)* | Sets the text of a run in the Rive scene |
