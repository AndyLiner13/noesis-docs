Source: https://www.noesisengine.com/docs/App.RiveBase._RiveInput.html

# RiveInput Class

## namespace [NoesisApp](/Gui.Core/_ClassHierarchy.md#noesisapp-namespace)

Represents an input value for the [RiveControl](/App.RiveBase/_RiveControl.md) state machine.

Each input is identified by a name in the rive scene. The input accepts bool or float values that can be set directly in the XAML or bound to properties of the data context.
...
| Name | Description |
| --- | --- |
| ● *InputName* | Gets or sets the input name |
| ● *InputPath* | Gets or sets the path to the nested artboard |
| ● *InputValue* | Gets or sets the input value |
...
RiveInput has no attached properties
