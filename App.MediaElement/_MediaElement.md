Source: https://www.noesisengine.com/docs/App.MediaElement._MediaElement.html

# MediaElement Class

## namespace [NoesisApp](/Gui.Core/_ClassHierarchy.md#noesisapp-namespace) | [MSDN](http://msdn.microsoft.com/en-us/library/system.windows.controls.mediaelement.aspx)
...
| Name | Description |
| --- | --- |
| ● *Balance* | Gets or sets the balance between the left and right speaker volumes. Represented in a range between -1 (left) and 1 (right). The default is 0. |
| ● *IsMuted* | Gets or sets a value that indicates whether the media is muted. |
| ● *LoadedBehavior* | Gets or sets the load behavior MediaState for the media. The default value is Play. |
| ○ *Position* | Gets or sets the current position of the media (seek operations). |
| ● *ScrubbingEnabled* | Gets or sets a value that indicates whether the [MediaElement](/App.MediaElement/_MediaElement.md) will update frames for seek operations while paused. The default is false. |
| ● *Source* | Gets or sets the media source. |
| ○ *SpeedRatio* | Gets or sets the ratio of speed that media is played at. Represented by a value between 0 and the largest double value. The default is 1.0. |
| ● *Stretch* | Gets or sets a value that describes how a [MediaElement](/App.MediaElement/_MediaElement.md) fills the destination rectangle. The default is Stretch.Uniform. |
| ● *StretchDirection* | Gets or sets a value that determines the restrictions on scaling that are applied to this media element. The default is StretchDirection.Both. |
| ● *UnloadedBehavior* | Gets or sets the unload behavior MediaState for the media. The default value is Close. |
...
MediaElement has no attached properties
...
|  *GetBufferingProgress()* | Gets the percentage of buffering completed for streaming content, represented in a value between 0 and 1. |
|  *GetDownloadProgress()* | Gets the percentage of download progress for content located at a remote server, represented by a value between 0 and 1. The default is 1. |
|  *GetNaturalDuration()* | Gets the natural duration of the media. The default is [Duration](/Gui.Animation/_Duration.md).Automatic. |
|  *GetNaturalVideoHeight()* | Gets the pixel height of the video. |
|  *GetNaturalVideoWidth()* | Gets the pixel width of the video. |
...
| Name | Description |
| --- | --- |
| ▸ *BufferingEnded* | Occurs when media loading has finished. |
| ▸ *BufferingStarted* | Events Occurs when media loading has finished. |
| ▸ *MediaEnded* | Occurs when the media has ended. |
| ▸ *MediaFailed* | Occurs when an error is encountered. |
| ▸ *MediaOpened* | Occurs when media loading has finished. |
